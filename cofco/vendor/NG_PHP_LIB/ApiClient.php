...
class InternalCall
{
     ...
     * @return mixed
     */
    protected function _doRestCall($method, $url, $data = array())
    {
        ...
        if(!empty($_COOKIE)){
            ...
            foreach($cookie_key as $c_item){
                if(strstr($c_item, '_com_AUTOCI_BRANCH')){
    ...
    public function call($url, $param = array(), $version, $is_debug, $method = 'GET', $time_out)
    {
        $this->setDebugEnabled($is_debug);
        $this->setTimeOut($time_out);
        $this->setVersion($version);

        $result = $this->_doRestCall($method, $url, $param);
        if($is_debug){
...
class ApiClient
{
     ...
     * @return array
     */
    static function doServiceCall($service_name, $api, $api_version, $get_params = array(), $post_params = array(), $extend_config = array())
    {
        ...
        if($service){
            ...
            $temp_ret = self::$_request->call($url, $post_params, $api_version, $is_debug, $method, $time_out);
