...
//                    }
    ...
    {
        if ( ! $order_info || empty($wms_order_info['extend_data'])) {
            return false;
        }
        ...
        $extendFunction = function ($order, $add_extend_data) {
            ...
            return json_encode(array_merge($add_extend_data, $extend_data), JSON_UNESCAPED_UNICODE);
        };
        ...
        foreach ($orderList as $order) {
            OrderModel::OrderUpdate(['order_id' => $order->order_id], ['extend_data' => $extendFunction($order, $add_extend_data)]);
        }
    }
}
