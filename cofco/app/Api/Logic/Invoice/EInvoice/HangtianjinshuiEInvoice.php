...
class HangtianjinshuiEInvoice
{
    ...
    protected function setSoapInfo($data = array(), $method)
    {
        if (empty($data) || !is_array($data)) {
            throw new \Exception('转换xml参数不正确', 400);
        }
        //todo 将数组转为xml
    ...
    protected function request($header = array(), $str = '')
    {
        ...
        Logger::General('einvoice_hangtianjinshui', array(
            'request_header' => $header,
            'domain' => $this->request_config['domain'],
            'httpCode' => $curl->GetHttpCode(),
            'error' => $curl->GetError(),
            'request_xml_content' => $str,
            'request' => $_result
        ));

        $tmpArr1 = explode('<return>', $_result);
        if (!$tmpArr1[1]) {
            throw new \Exception('请求第三方远程信息失败!', 500);
        }
