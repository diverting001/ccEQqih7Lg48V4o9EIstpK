...
use App\Api\Logic\Invoice\Perform\SalyutPerform;
/**
 * 发票平台V2 Logic
 *
 * @package     api
...
class InvoicePerform
{
    ...
    public function apply($applyInfo, & $errMsg = '')
    {
        ...
        if (in_array($applyInfo['perform'], $this->_openapiPerform)) {
            ...
            return true;
        } else if (in_array($applyInfo['perform'], $this->_salyutPerform)) {
            $salyutPerformLogic = new SalyutPerform();

            $ret = $salyutPerformLogic->apply($applyInfo, $errMsg);
