...
namespace App\Api\V3\Service\Calculate;
...
class Calculate
{
    ...
    private function pointProsess()
    {
        ...
        $withRuleRes = $this->serviceLogic->ServiceCall('member_point_with_rule', $withRuleParm);
        ...
        $allMemeberAccount = $this->getMemberPoint($this->memberId, $this->companyId, $pointChannel);

        foreach ($this->assetsList['point']['use_point'] as $sceneId => $curPointData) {
            //当前场景账户余额
            $curMemberPoint     = is_numeric($allMemeberAccount[$sceneId]['point']) ? $allMemeberAccount[$sceneId]['point'] : 0;
            ...
            $this->resultData['discount']['point_money'][$sceneId] = [
                'scene_id'    => $sceneId,
                'point_money' => $this->point2money($sceneMaxDiscountPoint, $pointChannel)
            ];
            ...
            foreach ($this->productList as $bn => $productInfo) {
                ...
                $goodsUsePointMoney = $this->point2money($goodsUsePoint, $pointChannel);
                $goodsUsePoint      = $this->money2point($goodsUsePointMoney, $pointChannel);
        ...
        \Neigou\Logger::Debug("calculate.debug.split", array(
            ...
            "response_result" => $allMemeberAccount,
