...
class Voucher
{
    ...
    public function createVoucher($create_params, &$message)
    {
        ...
        // 内购券过期时间统一使用过期当日最后一秒，即过期当日23:59:59。
        $create_params['valid_time'] = strtotime(date("Y-m-d", $create_params['valid_time']) . "23:59:59");
        $create_time = time();
        $b_create_suc = true;
        $op_data = array(
...
//                        var_dump($ret);die;
    ...
    {
        \Neigou\Logger::General('action.voucher', array('action' => 'queryOrderVoucher', 'order_id' => $order_id));
    ...
    {
        \Neigou\Logger::General('action.voucher', array('action' => 'queryMemberVoucher', 'member_id' => $member_id));
        $where = [
            ['member_id', $member_id],
            ['disabled', 0]
        ];
        $order_voucher_list = $this->_db->table($this->promotion_voucher_order_user)->where($where)->get()->all();
        if (empty($order_voucher_list)) {
            \Neigou\Logger::General(
                'action.voucher',
                array(
            ...
            foreach ($order_voucher_list as $order_voucher_item) {
                ...
                if ($result->valid_time < $from_time) {
                    continue;
                }
                ...
                foreach ($order_voucher_item as $key => $value) {
                    $result->$key = $value;
                }
                $voucher_data_list[] = $result;
            }
...
//            $message = "创建失败";
    ...
    {
        \Neigou\Logger::General('action.voucher', array('action' => 'queryBlackList', 'data' => $create_data_array));
//        $create_data_array = json_decode($json_data, true);
        if (empty($create_data_array) ||
            !isset($create_data_array['type']) ||
            !isset($create_data_array['rule']) ||
