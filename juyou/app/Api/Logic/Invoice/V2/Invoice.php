...
class Invoice
{
    ...
    public function saveInvoiceDetail($applyId, $type, $invoiceDetail, & $errMsg = '')
    {
        ...
        // 申请状态(1:待处理  2:进行中 3:完成 4:异常 5:已提交待返回 6.换开已作废待重新提交 7.换开已提交待返回 0 取消)
        if ($invoiceApplyInfo['apply_status'] == 3 OR $invoiceApplyInfo['apply_status'] == 0) {
            return true;
        }
    ...
    public function fixApplyData($applyId, $items, & $errMsg = '')
    {
        ...
        // 申请状态(1:待处理  2:进行中 3:完成 4:异常 5:已提交待返回 6.换开已作废待重新提交 7.换开已提交待返回)
        if (!in_array($invoiceApplyInfo['apply_status'], array(0, 1, 2, 4))) {
            $errMsg = '状态不允许修改';
            return false;
        }
    ...
    {
        $culr = new \Neigou\Curl();
        $content    = $culr->Get($file);
        $tempFile = tempnam(sys_get_temp_dir().'/form_test', 'InvoiceFormTemp');
        file_put_contents($tempFile, $content);
        $file = new UploadedFile($tempFile, basename($tempFile), 'text/plain');
        $service_logic = new Service();

        $res =  $service_logic->ServiceCall('image_upload',[],null,array('file'=>$file),['path'=>'einvoice']);
        //清空暂时文件
        @unlink($tempFile);
        return $res['data']['url'];
    }
