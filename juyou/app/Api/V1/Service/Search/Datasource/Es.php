...
class Es
{
    ...
    public function Select($request_data)
    {
        ...
        if ($respnse_data === false || isset($respnse_data['error'])) {
            //执行失败,记录
            \Neigou\Logger::General('es_query_error', array('result' => json_encode($respnse_data), 'sender' => json_decode($this->query_data)));
            return false;
        } else {
            //聚合数据
            $aggs_list = array();
...
//        if ($pattern == '_search') {
//            app('request')->businessData['error_msg'] = [$this->query_data];
//            app('request')->businessData['debug_msg'] = [$post_url];
//        }
//        echo $dsl, "\r\n";
        \Neigou\Logger::General('es_query', array(
            'post_url' => $post_url,
            'dsl' => $dsl,
        ));
    ...
    public function ParseSearchAggsData($data)
    {
        ...
        if (!empty($data['aggregations'])) {
            foreach ($data['aggregations'] as $field => $aggs) {
                switch ($field) {
                    case 'cat_list':
                        if (!empty($aggs['cat_level_0']['cat_level_1']['buckets'])) {
                            foreach ($aggs['cat_level_0']['cat_level_1']['buckets'] as $k => $v) {
                                $lever_1_son = array();
