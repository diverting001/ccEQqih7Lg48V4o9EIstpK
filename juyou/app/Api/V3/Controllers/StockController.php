...
use App\Api\V3\Service\Stock\BranchStock as BranchStock;
use App\Api\V3\Service\Stock\StockRestrict as StockRestrict;
...
class StockController extends BaseController
{
    ...
    function GetProudctStock(Request $request)
    {
        ...
        $channel = trim($content_data['environment']['project_code']);
        if (empty($channel)) {
            $channel = 'EC';
        }
        ...
        foreach ($product_list as $product) {
            $product_num = empty($product['product_num']) ? 1 : $product['product_num'];
            $product_bn_list[] = $product['product_bn'];
            $product_num_list[] = $product_num;
            $bn_num_mapping[$product['product_bn']] = $product_num;
        }
        $province = !isset($filter['province']) ? '' : trim($filter['province']);
        $city = !isset($filter['city']) ? '' : trim($filter['city']);
        $county = !isset($filter['county']) ? '' : trim($filter['county']);
        $town = !isset($filter['town']) ? '' : trim($filter['town']);
        $addr = !isset($filter['addr']) ? '' : trim($filter['addr']);
        $extend_data = !isset($filter['extend_data']) ? '' : trim($filter['extend_data']);
        ...
        foreach ($this->_server_stock_obj as $server_obj) {
            //获取服务库存
            $product_service_stock_list = $server_obj->GetStock($product_bn_list, [
                'channel' => $channel,
                'province' => $province,
                'city' => $city,
                'county' => $county,
            ...
            //组织数据
            foreach ($product_bn_list as $bn) {
                $main_stock = 0;
                $stock = 0;
                if (isset($product_service_stock_list[$bn])) {
                    if ($product_stock_list[$bn]) {
                ...
                $stock_state = $stock >= $bn_num_mapping[$bn] ? 1 : 2;
                $product_stock_list[$bn] = [
                    'product_bn' => $bn,
                    'main_stock' => $main_stock,
                    'stock' => $stock,
                    'stock_state' => $product_service_stock_list[$bn]['stock_state'] ? $product_service_stock_list['stock_state'] : $stock_state,
                ];
            }
        }
     ...
     * @todo 库存处理对象
     */
    public function SetStockObj(\App\Api\V3\Service\Stock\Stock $obj)
    {
