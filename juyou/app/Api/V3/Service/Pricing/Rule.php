...
use App\Api\V3\Service\Pricing\Ternary as Ternary;
use App\Api\V3\Service\Pricing\Express as Express;
class Rule{
    //商品价格规则映射
...
//                            'branch_list'   => $branch_price['branch_list'],
//                            'product_bn'   => $product['product_bn'],
                        );
                        //对应关系
...
//            'exclusive'  => $rule['exclusive'], //是否排除当前场景 0:不除 1:排除
                ...
                case 4: //表达式定价
                    if(isset($rule['rule']['data']['express'])){
                        ...
                        if($expressStr !== false){
                            try{
                                //解析判断三元运算符
                                $terNaryObj = new Ternary();
                                if($terNaryObj->isTernary($expressStr)){
                                    $lastPrice = $terNaryObj->compute($expressStr);
                                }else{
                                    $express = new Express();
                                    $lastPrice = $express->startCalculate($expressStr);
                                }
     ...
     * @todo 创建商品规则树
     */
    protected function CreateRuleTree($type){
        //无数据时重新创建
        $self_rule_mapping = $this->getRuleMapping($type);
...
//            }
    ...
    protected function GetProdcutRange($range_id){
        ...
        $range_info  = app::get('b2c')->model('pricing_range')->GetRangeInfo($range_id);
        if(empty($range_info)) return $range_data;
        switch ($range_info['type']){
            case '1' :
                break;
            case '2' :
                $range_brand_list   = app::get('b2c')->model('pricing_range')->GetRangeBrand($range_id);
                ...
                break;
            case '4':
                $range_brand_list   = app::get('b2c')->model('pricing_range')->GetRangeProduct($range_id);
                if(!empty($range_brand_list)){
                ...
                break;
            case '5' :
                $range_supplier_list   = app::get('b2c')->model('pricing_range')->GetRangeSupplier($range_id);
                ...
                break;
            case '6' :
                $range_supplier_list   = app::get('b2c')->model('pricing_range')->GetRangeMall($range_id);
                if(!empty($range_supplier_list)){
                ...
                break;
            case '7' :
                $range_container_list   = app::get('b2c')->model('pricing_range')->GetRangeContainer($range_id);
    ...
    protected function GetStage($stage_id){
        ...
        $stage_info  = app::get('b2c')->model('pricing_stage')->GetStageInfo($stage_id);
        ...
        switch ($stage_info['type']){
            case '1':
                ...
                break;
            case  '3':
                $stage_channel_list   = app::get('b2c')->model('pricing_stage')->GetStageChannel($stage_id);
                ...
                break;
            case '4':
                $stage_company_list   = app::get('b2c')->model('pricing_stage')->GetStageCompany($stage_id);
                ...
                break;
            case '5':
                $stage_company_tag_list   = app::get('b2c')->model('pricing_stage')->GetStageCompanyTag($stage_id);
    ...
    public function GetRuleVersion(){
        $rule_version  = app::get('b2c')->model('pricing_rule')->GetRuleVersion();
        return $rule_version;
    }
