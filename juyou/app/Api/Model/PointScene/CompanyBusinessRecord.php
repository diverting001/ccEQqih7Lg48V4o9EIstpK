...
//            ->where($parse_where)
    ...
    public static function queryCompanyAccountRecord($companyIds,$page,$page_size){
        $obj = app('api_db')->table('server_new_point_company_scene_rel as com')
            ...
            ->select('com.company_id',app('api_db')->raw('sum(point) as point'),app('api_db')->raw('sum(used_point) as sum_used_point'),app('api_db')->raw('sum(point-used_point) as balance'))
            ->forPage($page , $page_size)
