...
class CostReachedRuleProcessor extends BasicRuleProcessor
{
    ...
    public function match ($rule_condition, $filter_data) {
        ...
        \Neigou\Logger::General('action.voucher', array('action'=>'CostReachedRuleProcessor',
            'limit_cost'=>$rule_condition['limit_cost'], 'total_amount'=>$total_amount));
        if ($total_amount <=0) {
            return false;
        }
        ...
        // if($total_amount > $rule_condition['limit_cost'] ||  abs($total_amount - $rule_condition['limit_cost']) < 0.0001){
        //     return true;
        // }
        // return false;
    }
