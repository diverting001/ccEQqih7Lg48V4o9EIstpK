...
class AfterSale
{
    ...
    public function update($where = array(), $param = array())
    {
        ...
        $ret = $this->_db->table('server_after_sales')->where($where)->update($param);
        if (isset($where['after_sale_bn']) && isset($param['status'])) {
            $this->publishMessage($where['after_sale_bn']);
        }
    ...
    public function getAllAfterSaleOrder($orderId = 0, $productBn = 0)
    {
        ...
        $where1 = function ($query) use($orderId,$productBn){
            $query->where(array('order_id'=>$orderId,'product_bn' => $productBn))->whereIn('status',array(1,2,4,5,6,9,10,11,12,14));
        };
        ...
        return $res = $this->_db->table('server_after_sales')->where($where1)->orWhere($where2)->orWhere($where3)->sum('product_num');
    ...
    public function getAllRootOrderSaleNum($rootPid = 0, $productBn = '')
    {
        ...
        $sql = "SELECT sum(product_num) as roo_total_product_num FROM server_after_sales WHERE (root_pid = ? AND product_bn = ? AND status IN (1,2,4,5,6,9,10,11,12,14)) OR (root_pid = ? AND product_bn = ?  AND status = 7 and is_reissue = 1) OR (root_pid = ? AND product_bn = ? AND status = 13 and is_reissue = 1);";

        $res = $this->_db->select($sql,array($rootPid,$productBn,$rootPid,$productBn,$rootPid,$productBn));
