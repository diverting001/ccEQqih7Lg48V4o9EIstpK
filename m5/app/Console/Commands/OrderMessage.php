...
class OrderMessage extends Command
{
    ...
    public function testOrderPay() {
        ...
        $data   = ['order_id'=>"202502270424165098"];
        if (empty($data['order_id'])) {
            return false;
        }
        //获取订单
        $order_info = OrderModel::GetOrderInfoById($data['order_id']);
        if ($order_info->status == 2) { // 如果订单已取消，加入异常消息
            $exception_data = [
                'business' => 'order',
                'data_id' => $data['order_id'],
                'wms_code' => $order_info->wms_code,
                'type' => 2,
                'data_create_time' => $order_info->create_time,
                'pop_owner_id' => $order_info->pop_owner_id,
                'company_id' => $order_info->company_id,
            ];
            $service_logic = new Service();
            $res = $service_logic->ServiceCall('exception_msg_update', $exception_data);
            echo $res['error_code'];
        }

        // 检查资产状态
        $service_logic = new Service();
        $result = $service_logic->ServiceCall('asset_get', array('use_type' => 'ORDER', 'use_obj' => $data['order_id'], 'sync_query' => 1));
        if ($result['error_code'] != 'SUCCESS' OR empty($result['data']) OR $result['data']['status'] != 3)
        {
            \Neigou\Logger::General('pay_order_confirm', array('action' => 'asset_check', 'data' => $data, 'result' => $result));
        }

        // 判断订单是否自动完成
        $order_extend_data = $order_info->extend_data ? json_decode($order_info->extend_data, true) : array();
        $auto_confirm = isset($order_extend_data['auto_confirm']) ? $order_extend_data['auto_confirm'] : 1;
        ...
        $confirmLogic = new OrderConfirm();

        $confirmStatusInfo = $confirmLogic->getConfirmStatus($data);

        $auto_confirm = $confirmStatusInfo['auto_confirm'] == 0 ? 0 : $auto_confirm;
        $is_manual = $confirmStatusInfo['is_manual'] == 0 ? 0 : 1;
    ...
    public function UpdateOrder()
    {
        $function = function ($data) {
            // 检查缓存
            if ($data['order_id']) {
                $key = 'ORDER_MESSAGE_UPDATE_' . $data['order_id'];
                $redis = new \Neigou\RedisClient();
                if ($redis->_redis_connection->get($key) !== false) {
                    return true;
                }
                // 记录缓存
                $redis->_redis_connection->setex($key, 1, 1);
            }
            //日志记录
            $funcgion_log = function ($method_name, $success, $msg) use ($data) {
                $log_array = array(
            ...
            // 更新扩展信息
            $this->updateOrderExtendData($order_info, $wms_order_info);
            return $res;
        };

        $retryFunction = function ($data) use ($function) {
            $reCont = 0; // 子单同时发货时，后来的订单发货状态变更出错。故重试
            $res = false;
            while ($reCont < 3 && !$res) {
                $reCont++;
                $res = call_user_func($function, $data);
                if (!$res) {
                    echo json_encode($data['data']) . '重试 ' . $reCont . "\n";
                    sleep(1); // 当时间变动后再重试可以避免last_modified相同导致的冲突
                }
            }
            return $res;
        };
        $amqp = new \Neigou\AMQP();
        $amqp->ConsumeMessage('service_order_update', 'service', 'order.update.*', $retryFunction, true);
    }
            ...
            if (isset($order_item_list[$order_id])) {
                foreach ($order_item_list[$order_id] as $order_item) {
                    $items[] = [
                        ...
                        'p_bn' => $order_item->p_bn,
                    ];
                }
            }
            $extendData = !empty($order_info->extend_data) ? (json_decode($order_info->extend_data) ? json_decode($order_info->extend_data,
                true) : $order_info->extend_data) : [];
            $extendData['pay_real_name'] = $pay_log->pay_name;
            $extendData['pay_trade_no'] = $pay_log->trade_no;
            $extendData['pay_payment_id'] = $pay_log->payment_id;
...
//                return true;
    ...
    protected function OrderSplit($order_info, $wms_order_info)
    {
        ...
        if (empty($wms_order_info['son_orders'])) {
            ...
            foreach ($wms_order_info['items'] as $item) {
                ...
                $key = self::_createUniqueProductBn($item['product_bn'], $item_type, $p_bn);
                if (!isset($order_items[$key])) {
                    echo '履约商品和原订单商品不一致';
                    return false;
                }
                if ($order_items[$key]->nums != $item['nums']) {
                    //$wms_order_info['son_orders']   = $wms_order_info;
                    //break;
                    echo '履约商品和原订单商品不一致2';
                    return false;
                }
                $order_items[$key]->nums = $order_items[$key]->nums - $item['nums'];
            }

            // 检查商品数量
            foreach ($order_items as $item) {
                if ($item->nums != 0) {
                    \Neigou\Logger::General('server_order_message_nums_error', array('remark' => 'main', 'data' => array('order_info' => $order_info, 'wms_order_info' => $wms_order_info)));
                    break;
                }
            }
        }
        ...
        $deliveryOrderData = []; //处理发货和修改物流的日志
        if (!empty($wms_order_info['son_orders'])) {
            $valid_split_order = [];
            //已拆分子订单
            foreach ($wms_order_info['son_orders'] as $son_order) {
                ...
                if (isset($split_delivery_bn[$son_order['delivery_id']])) {
                    ...
                    if (!empty($save_data)) {
                        ...
                        $deliveryOrderData[] = [
                            'save_data' => $save_data,
                            'order' => (array)$split_delivery_bn[$son_order['delivery_id']]
                        ];
                    }
            ...
            foreach ($order_items as $item) {
                if ($item->nums != 0) {
                    \Neigou\Logger::General('server_order_message_nums_error', array('remark' => 'sub', 'data' => array('order_info' => $order_info, 'wms_order_info' => $wms_order_info)));
                    break;
                }
            }
            ...
            if (!empty($new_split_order)) {
                $fixErrMsg = '';
                $fixRes = $this->checkAndResetSplitProductPointPay($order_info, $new_split_order, $valid_split_order, $fixErrMsg);
                if (!$fixRes) {
                    echo '修正错误：' . $fixErrMsg;
                    return false;
                }
        ...
        if ($res) {
            ...
            $OrderChangeLog->saveOrderLog($deliveryOrderData);
        }
        return $res;
    }
    ...
    protected function SplitOrderGenerate($order_info, $son_order, &$send_mq_list)
    {
        ...
        foreach ($son_order['items'] as $item) {
            ...
            $order_info_itme = $order_info->items[$key];
        ...
        // 订单已取消
        if ($new_split_order['status'] == 2) {
            $new_split_order['cancel_time'] = time();
            //订单完成消息
            $send_mq_list[] = [
                'type' => 'cancel',
...
//                    }
    ...
    /**
     * @param $order_info
     * @param $wms_order_info
     *
     * @return bool
     */
    public function updateOrderExtendData($order_info, $wms_order_info)
    {
        if (!$order_info) {
            return false;
        }

        if (empty($wms_order_info['extend_data'])) {
            $wms_order_info['extend_data'] = [];
        }

        $add_extend_data = $wms_order_info['extend_data'];

        $add_extend_data = is_array($add_extend_data) ? $add_extend_data : json_decode($add_extend_data, true);

        if ( ! is_array($add_extend_data)) {
            return false;
        }

        $extendFunction = function ($order, $add_extend_data) {
            $extend_data = ! empty($order->extend_data) ? (is_array($order->extend_data) ? $order->extend_data : json_decode($order->extend_data,
                true)) : [];
            ...
            $new_extend_data = array_merge($extend_data, $add_extend_data);
            if (!isset($new_extend_data['supplier_order_bn'])) {
                $new_extend_data['supplier_order_bn'] = $order->wms_order_bn;
            }

            return json_encode($new_extend_data, JSON_UNESCAPED_UNICODE);
        };

        OrderModel::OrderUpdate(['order_id' => $order_info->order_id],
            ['extend_data' => $extendFunction($order_info, $add_extend_data)]);

        $orderList = OrderModel::GetOrderListByPid($order_info->order_id);

        foreach ($orderList as $order) {
            // 父订单上面已经更新过了，无需再次更新
            if ($order->order_id == $order_info->order_id) {
                continue;
            }
            $son_add_extend_data = $add_extend_data;
            foreach ($wms_order_info['son_orders'] as $son_wms_order_info) {
                if ($son_wms_order_info['delivery_id'] == $order->wms_delivery_bn) {
                    $son_add_extend_data = array_merge($son_add_extend_data, (array)$son_wms_order_info['extend_data']);
                    break;
                }
            }
            OrderModel::OrderUpdate(['order_id' => $order->order_id], ['extend_data' => $extendFunction($order, $son_add_extend_data)]);
        }
    }
    ...
    /** 修复混合支付导致的精度问题 */
    private function checkAndResetSplitProductPointPay($orderInfo, &$newSplitOrder, $validSplitOrder, &$errMsg)
    {
        if (bccomp($orderInfo->point_amount, '0', 2) === 0) {
            // 无积分支付数据无需修复
            return true;
        }

        // 商品对应的积分支付金额
        $diffProductPointAmount = array();
        // 计算总的商品对应的积分支付金额
        foreach ($orderInfo->items as $key => $item) {
            $diffProductPointAmount[$key] = $item->point_amount;
        }

        // 扣除已经存在订单的积分支付金额
        $validOrderIds = array_column($validSplitOrder, 'order_id');
        $validOrderItems = OrderModel::GetOrderItems($validOrderIds);
        foreach ($validOrderItems as $subOrder) {
            foreach ($subOrder as $item) {
                $key = self::_createUniqueProductBn($item->bn, $item->item_type, $item->p_bn);
                if (!isset($diffProductPointAmount[$key])) {
                    \Neigou\Logger::General('server_order_message_fix_point', array('action' => 'error_valid_order_point_key', 'data' => $diffProductPointAmount, 'sparam1' => json_encode($newSplitOrder), 'sparam2' => json_encode($orderInfo), 'sparam3' => json_encode($validSplitOrder), 'sparam4' => $item, 'sparam5' => $key,));
                    $errMsg = '已有订单商品积分支付金额不存在' . $key;
                    return false;
                }
                $diffProductPointAmount[$key] = bcsub($diffProductPointAmount[$key], $item->point_amount, 2);
            }
        }

        // 扣除新拆单的积分支付金额
        foreach ($newSplitOrder as $newSubOrder) {
            foreach ($newSubOrder['items'] as $item) {
                $key = self::_createUniqueProductBn($item['bn'], $item['item_type'], $item['p_bn']);
                if (!isset($diffProductPointAmount[$key])) {
                    \Neigou\Logger::General('server_order_message_fix_point', array('action' => 'error_new_order_point_key', 'data' => $diffProductPointAmount, 'sparam1' => json_encode($newSplitOrder), 'sparam2' => json_encode($orderInfo), 'sparam3' => json_encode($validSplitOrder), 'sparam4' => $item, 'sparam5' => $key,));
                    $errMsg = '新拆单商品积分支付金额不存在' . $key;
                    return false;
                }
                $diffProductPointAmount[$key] = bcsub($diffProductPointAmount[$key], $item['point_amount'], 2);
            }
        ...
        // 检查是否有商品需要修正数据。如果 新拆单的订单内的积分支付+已存在订单内的积分支付 != 总订单的积分支付  则需要修正数据
        $needFix = false;
        $maxAllowFixMoney = '1'; // 以订单拆单导致单个商品积分支付差额无法达到1元暂作为本方法的兜底措施，先不让订单进行拆分，等到人工确定确实存在该情况，则调整或者去除本限制
        foreach ($diffProductPointAmount as $item) {
            // 不存在价格差了特别大的商品
            if (bccomp($item, $maxAllowFixMoney, 2) >= 0) {
                \Neigou\Logger::General('server_order_message_fix_point', array('action' => 'error_too_large', 'data' => $diffProductPointAmount, 'sparam1' => json_encode($newSplitOrder), 'sparam2' => json_encode($orderInfo), 'sparam3' => json_encode($validSplitOrder), 'sparam4' => $item, 'sparam5' => $maxAllowFixMoney,));
                $errMsg = '金额过大' . $item . ' >= ' . $maxAllowFixMoney;
                return false;
            }
            // 存在价格不等于0的商品，则需要修复积分支付数据
            if (bccomp($item, '0', 2) !== 0) {
                $needFix = true;
            }
        }
        if (!$needFix) {
        ...
        \Neigou\Logger::General('server_order_message_fix_point', array('action' => 'start', 'data' => $diffProductPointAmount, 'sparam1' => json_encode($newSplitOrder), 'sparam2' => json_encode($orderInfo), 'sparam3' => json_encode($validSplitOrder),));

        // 修复拆单中的积分支付和现金支付数据
        foreach ($newSplitOrder as &$newSubOrder) {
            foreach ($newSubOrder['items'] as &$subOrderItem) {
                $key = self::_createUniqueProductBn($subOrderItem['bn'], $subOrderItem['item_type'], $subOrderItem['p_bn']);
                if (!isset($diffProductPointAmount[$key]) || bccomp($diffProductPointAmount[$key], '0', 2) === 0) {
                    continue;
                }
                $diffAmount = $diffProductPointAmount[$key];

                if (bccomp($diffAmount, '0', 2) > 0) {
                    // 正数，证明新拆分的订单商品积分支付金额少了，补积分，扣现金。扣现金不能出现负数
                    $diffNum = bccomp($diffAmount, $newSubOrder['cur_money'], 2) > 0 ? $newSubOrder['cur_money'] : $diffAmount;
                    $subOrderItem['point_amount'] = round(bcadd($subOrderItem['point_amount'], $diffNum, 2), 2);
                    $newSubOrder['point_amount'] = round(bcadd($newSubOrder['point_amount'], $diffNum, 2), 2);
                    $newSubOrder['cur_money'] = round(bcsub($newSubOrder['cur_money'], $diffNum, 2), 2);
                    $newSubOrder['payed'] = round(bcsub($newSubOrder['payed'], $diffNum, 2), 2);
                    $diffProductPointAmount[$key] = bcsub($diffAmount, $diffNum, 2);
                } else {
                    // 负数，证明新拆分的订单商品积分支付金额多了，扣积分，补现金。扣积分不能出现负数
                    $absAmount = bcmul($diffAmount, '-1', 2);
                    $diffNum = bccomp($absAmount, $newSubOrder['point_amount'], 2) > 0 ? $newSubOrder['point_amount'] : $absAmount;
                    $subOrderItem['point_amount'] = round(bcsub($subOrderItem['point_amount'], $diffNum, 2), 2);
                    $newSubOrder['point_amount'] = round(bcsub($newSubOrder['point_amount'], $diffNum, 2), 2);
                    $newSubOrder['cur_money'] = round(bcadd($newSubOrder['cur_money'], $diffNum, 2), 2);
                    $newSubOrder['payed'] = round(bcadd($newSubOrder['payed'], $diffNum, 2), 2);
                    $diffProductPointAmount[$key] = bcadd($diffAmount, $diffNum, 2);
                }
            }
        }

        // 检查是否完全修复
        foreach ($diffProductPointAmount as $diff) {
            if (bccomp($diff, '0', 2) !== 0) {
                \Neigou\Logger::General('server_order_message_fix_point', array('action' => 'error_fix_end_check', 'data' => $diffProductPointAmount, 'sparam1' => json_encode($newSplitOrder), 'sparam2' => json_encode($orderInfo), 'sparam3' => json_encode($validSplitOrder), 'sparam4' => $diff,));
                $errMsg = '没有完整修复拆单数据';
                return false;
            }
        }

        \Neigou\Logger::General('server_order_message_fix_point', array('action' => 'end', 'data' => $diffProductPointAmount, 'sparam1' => json_encode($newSplitOrder),));
        return true;
    }
}
