        ...
        if ($source === 'redis') {
            ...
            while (1) {
                ...
                while (count($goods_ids) < $page_size) {
                    ...
                    \Neigou\Logger::Debug('redis_voucher_msg', array('msg' => $msg));
                    $msgs[] = $msg;
                    $val = $msg;
                    //校验
                ...
                \Neigou\Logger::Debug('redis_voucher_goods', array('goods_ids' => $goods_ids));
    ...
    function is_match($goods, $rule)
    {
        ...
        switch ($rule['condition']['processor']) {
            ...
            case 'CostMallAndCatRuleProcessor': // 指定商城及类目
                ...
                if (!empty($goods['cats'])) {
                    foreach ($goods['cats'] as $cat) {
                        if (!empty($cat['cat_id1']) && !in_array($cat['cat_id1'], $goods_cat_ids)) {
                            $goods_cat_ids[] = $cat['cat_id1'];
                        }
                        if (!empty($cat['cat_id2']) && !in_array($cat['cat_id2'], $goods_cat_ids)) {
                            $goods_cat_ids[] = $cat['cat_id2'];
                        }
                        if (!empty($cat['cat_id3']) && !in_array($cat['cat_id3'], $goods_cat_ids)) {
                            $goods_cat_ids[] = $cat['cat_id3'];
                        }
                    }
                }
...
//                        $is_match = 0;
//                    } elseif (array_intersect($goods_cat_ids, $mall_cat_ids) && array_intersect($mall_ids, (array)$goods['moduled'])) {
//                        $is_match = 0;
//                    } else {
//                        $is_match = 1;
//                    }
                    if (in_array('all', $mall_ids)) {
                        $is_match = 0;
                    } elseif ((in_array('all', $mall_cat_ids) || array_intersect($goods_cat_ids, $mall_cat_ids)) && array_intersect($mall_ids, (array)$goods['moduled'])) {
                        $is_match = 0;
                    } else {
                        $is_match = 1;
                    }
            ...
            case 'CostShopRuleProcessor': // 指定店铺
                ...
                if ($rule['condition']['filter_rule']['use_type'] != 'unless' && array_intersect($shop_ids, $goods['shop_ids']) || $rule['condition']['filter_rule']['use_type'] == 'unless' && !array_intersect($shop_ids, $goods['shop_ids'])) {
                    $is_match = 1;
                }
                break;
        }
