...
class Elasticsearchcreateindex
{
    ...
    {
        if (empty($index_list) || !is_array($index_list)) {
            return false;
        }
            ...
            'mappings' => array(
                config('neigou.ESSEARCH_TYPE') => array(
                    ...
                    'properties' => array(
                        ...
                        //可见范围省
                        "region_visible_province" => array(
                            "type" => "integer"
                        ),
                        //可见范围市
                        "region_visible_city" => array(
                            "type" => "integer"
                        ),
                        //可见范围区县
                        "region_visible_area" => array(
                            "type" => "integer"
                        ),
                        //配送类型
                        "shipping_type" => array(
    ...
    {
        if (empty($branch_id)) {
            $type_name = config('neigou.ESSEARCH_TYPE');
        } else {
            $type_name = $this->getBranchTypeName($branch_id);
        }
        return $type_name;
    ...
    public function GetUseIndexList()
    {
        ...
        $cache_obj->fetch($this->_index_cache_prefix, $this->_index_version . '_' . $this->_index_cache_key,
            $index_name_list);
...
//
    ...
    public function SaveUseIndexList($use_index_list)
    {
        ...
        $cache_obj = kernel::single('base_sharedkvstore');
        $res = $cache_obj->store($this->_index_cache_prefix, $this->_index_version . '_' . $this->_index_cache_key,
            $use_index_list, 0);
    }
    ...
    {
        $res = $this->store($this->_index_cache_prefix, $this->_index_version . '_' . $this->_type_cache_key,
            $use_type_list, 0);
    }
