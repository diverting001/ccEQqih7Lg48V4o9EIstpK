...
class Es
{
    ...
    public function __construct()
    {
        ...
        if (PSR_PROVINCE_MAPPING !== 'PSR_PROVINCE_MAPPING') {
            $this->province = json_decode(PSR_PROVINCE_MAPPING, true);
        }
    }
...
//        }
        ...
        if (isset($filter_data['keyword']) && !empty($filter_data['keyword'])) {
            $this->query_data['query']['bool']['must']['bool']['should'][] = array(
                'match_phrase' => array(
                    'name' => array(
...
//            $this->query_data['query']['bool']['filter']['nested']['path'] = 'cats';
...
//                $this->query_data['query']['bool']['filter']['nested']['query']['bool']['filter'][]['terms']['cats.cat_id1'] = $filter_data['cat_level_1']['cat_id'];
                $cat_id_arr = is_array($filter_data['cat_level_1']['cat_id']) ? $filter_data['cat_level_1']['cat_id'] : [$filter_data['cat_level_1']['cat_id']];
                $nested_arr['nested']['query']['bool']['filter'][]['terms']['cats.cat_id1'] = $cat_id_arr;
            }
...
//                $this->query_data['query']['bool']['filter']['nested']['query']['bool']['filter'][]['terms']['cats.cat_name1.raw'] = $filter_data['cat_level_1']['cat_name'];
                $cat_name_arr = is_array($filter_data['cat_level_1']['cat_name']) ? $filter_data['cat_level_1']['cat_name'] : [$filter_data['cat_level_1']['cat_name']];
                $nested_arr['nested']['query']['bool']['filter'][]['terms']['cats.cat_name1.raw'] = $cat_name_arr;
            }
        }
...
//                $this->query_data['query']['bool']['filter']['nested']['query']['bool']['filter'][]['terms']['cats.cat_id2'] = $filter_data['cat_level_2']['cat_id'];
                $cat_id_arr = is_array($filter_data['cat_level_2']['cat_id']) ? $filter_data['cat_level_2']['cat_id'] : [$filter_data['cat_level_2']['cat_id']];
                $nested_arr['nested']['query']['bool']['filter'][]['terms']['cats.cat_id2'] = $cat_id_arr;
            }
...
//                $this->query_data['query']['bool']['filter']['nested']['query']['bool']['filter'][]['terms']['cats.cat_name2.raw'] = $filter_data['cat_level_2']['cat_name'];
                $cat_name_arr = is_array($filter_data['cat_level_2']['cat_name']) ? $filter_data['cat_level_2']['cat_name'] : [$filter_data['cat_level_2']['cat_name']];
                $nested_arr['nested']['query']['bool']['filter'][]['terms']['cats.cat_name2.raw'] = $cat_name_arr;
            }
        }
...
//                $this->query_data['query']['bool']['filter']['nested']['query']['bool']['filter'][]['terms']['cats.cat_id3'] = $filter_data['cat_level_3']['cat_id'];
                $cat_id_arr = is_array($filter_data['cat_level_3']['cat_id']) ? $filter_data['cat_level_3']['cat_id'] : [$filter_data['cat_level_3']['cat_id']];
                $nested_arr['nested']['query']['bool']['filter'][]['terms']['cats.cat_id3'] = $cat_id_arr;
            }
...
//                $this->query_data['query']['bool']['filter']['nested']['query']['bool']['filter'][]['terms']['cats.cat_name3.raw'] = $filter_data['cat_level_3']['cat_name'];
                $cat_name_arr = is_array($filter_data['cat_level_3']['cat_name']) ? $filter_data['cat_level_3']['cat_name'] : [$filter_data['cat_level_3']['cat_name']];
                $nested_arr['nested']['query']['bool']['filter'][]['terms']['cats.cat_name3.raw'] = $cat_name_arr;
            }
        }
        if (!empty($nested_arr)) {
            $nested_arr['nested']['path'] = 'cats';
            $this->query_data['query']['bool']['filter'][] = $nested_arr;
        }
        //所属国家
        ...
        // goods_bn筛选
        if (isset($filter_data['bn'])) {
            $this->query_data['query']['bool']['filter'][]['terms']['bn'] = $filter_data['bn'];
        }

        // 筛选预定义字段
        foreach (['int1', 'int2', 'int3', 'int4', 'int5', 'double1', 'double2', 'double3', 'double4', 'double5', 'str1', 'str2', 'str3', 'str4', 'str5'] as $es_field) {
    ...
    public function ParseAggs($filter_data)
    {
        ...
        if (isset($filter_data['cat_level_3']) && !empty($filter_data['cat_level_3'])) {
            if (!empty($filter_data['cat_level_3']['cat_id'])) {
                $aggs_filter['cat_level_3_id']['term']['cat_level_3.cat_id'] = intval($filter_data['cat_level_3']['cat_id']);
            }
        }
        // 商城筛选
        if (isset($filter_data['mall_id']) && !empty($filter_data['mall_id'])) {
        ...
        $this->query_data['aggs']['cat_list'] = array(
            'filter' => (object)null,
            'aggs' => array(
                'cat_level_0' => array(
                    'nested' => array(
...
//                        break;
                ...
                // 预定义字段排序
                if (in_array($order_key, ['int1', 'int2', 'int3', 'int4', 'int5', 'double1', 'double2', 'double3', 'double4', 'double5', 'str1', 'str2', 'str3', 'str4', 'str5'])) {
    ...
    public function Query($pattern, $index_name = '', $index_type = '')
    {
        ...
        if ($pattern == '_search') {
            \Neigou\Logger::Debug('es_query_data', array(
                'post_url' => $post_url,
                'query_data' => $this->query_data
            ));
        }
        $this->query_data = array();  //执行完成清空执行数组
        $this->curl_ojb->time_out = 3;
        $data = $this->curl_ojb->Post($post_url, $dsl);
        $return_data    = json_decode($data, true);
        if (empty($return_data)) {
            //执行失败,记录
            \Neigou\Logger::General('es_query_error', array('result' => $data,'sparam1' => $this->curl_ojb->GetError() , 'sender' => $post_url,'data'=>$dsl,'action'=>'v1_select'));
            return false;
        }
        return $return_data;
    }
    ...
    protected function getCatTreeList($catids) {
        if(empty($catids)) {
            return array() ;
        }
        $serviceObj = new Service() ;
        $post_data = array('filter' => array('cat_id' => $catids )) ;
        $res = $serviceObj->ServiceCall('cat_tree_list' ,$post_data);
        if( 'SUCCESS' == $res['error_code'] && !empty($res['data'])) {
           return $res['data'] ;
        }
        return array() ;
    }
    protected function getBrandList($brandids)  {
        if(empty($brandids)) {
            return array() ;
        }
        $serviceObj = new Service() ;
        $post_data = array('filter' => array('brand_id' => $brandids )) ;
        $res = $serviceObj->ServiceCall('brand_list' ,$post_data);
        if( 'SUCCESS' == $res['error_code'] && !empty($res['data']['list'])) {
            $data = []  ;
            foreach ($res['data']['list'] as $datum) {
                $data[$datum['brand_id']] = $datum['brand_name'];
            }
            return $data ;
        }
        return array() ;
    ...
    public function ParseSearchAggsData($data)
    {
        ...
        if (!empty($data['aggregations'])) {
            foreach ($data['aggregations'] as $field => $aggs) {
                switch ($field) {
                    case 'cat_list':
                        if (!empty($aggs['cat_level_0']['cat_level_3']['buckets'])) {
                            $lever_3_catids = array_column($aggs['cat_level_0']['cat_level_3']['buckets'] ,'key');
                            $new_data['cat_list'] = $this->getCatTreeList($lever_3_catids);
                        } else {
                    ...
                    case 'brand':
                        if (!empty($aggs['brand_id']['buckets'])) {
                            $brand_ids = array_column($aggs['brand_id']['buckets'] ,'key') ;
                            $brand_list_info = $this->getBrandList($brand_ids) ;
                            foreach ($aggs['brand_id']['buckets'] as $k => $v) {
                                $new_data['brand'][] = array(
                                    'brand_name' => isset($brand_list_info[$v['key']])  ? $brand_list_info[$v['key']] : '',
                                    'brand_id' => $v['key'],
                                );
                            }
                        } else {
...
//            );
    ...
    private function sortByKeyword($keyword)
    {
        ...
        for ($i = 0, $count = count($keywordRules); $i < $count; $i++) {
            ...
            if (in_array($keywordRule['es_field'], ['brand_id', 'cat_level_1.cat_id', 'cat_level_2.cat_id', 'cat_level_3.cat_id'])) {
...
//            $pricing_stagees_mapping = $stage_es_mdl->GetAll();
            ...
            $pricing_stagees_mapping = $this->_db_store->table('server_pricing_stage_es_mapping')->get()->map(function ($value) {
                return (array)$value;
            })->toArray();;
