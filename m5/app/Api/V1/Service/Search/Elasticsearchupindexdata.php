...
class Elasticsearchupindexdata
{
    ...
    public function SaveElasticSearchsData($goods_ids, $branch_list = array())
    {
        ...
        if (!empty($split_goods_list['standard'])) {
            $split_goods_list['standard'] = $outletService->putOutletData($split_goods_list['standard']);
        ...
        if (!empty($split_goods_list['vbranch'])) {
            $split_goods_list['vbranch'] = $outletService->putOutletData($split_goods_list['vbranch']);
...
//        \Neigou\Logger::General('es.update.dml', $put_data_str);
    ...
    /**
     * 按指定条件，筛选更新es 中nested为文档的指定字段
     * @param array $goods_list nested待更新数据
     * @param string $nested_path nested对应字段名字
     * @param string $nested_id nested用来做判断的字段
     * @return bool|mixed|string
     */
    public function EsNestedFieldUpdate(array $goods_list,string $nested_path,string $nested_id)
    {
        if (empty($goods_list)) {
            return false;
        }
        ...
        $put_data_str = '';
        foreach ($goods_list as $goods) {
            $branch_id = intval($goods['branch_id']);
            $index_type = $es_index->GetIndexTypeName($goods['branch_id']);
            $time_local = date('c');
            $inex_type_id = [
                'update' => [
            ...
            //def targets = ctx._source.outlet_list.findAll(outlet_list -> outlet_list.outlet_id == params.outlet_id); for(e in targets) { e.outlet_logo = '1.jpeg' }
            $script = "def targets = ctx._source.{$nested_path}.findAll({$nested_path} -> {$nested_path}.{$nested_id} == params.{$nested_id});";
            $script .= "for(e in targets){";
            foreach ($goods['script'] as $tmp_k => $tmp_v) {
                if (is_string($tmp_v)) {
                    $script .= "e.{$tmp_k}='$tmp_v';";
                } else {
                    $script .= "e.{$tmp_k}=$tmp_v;";
                }
            }
            $script .= "ctx._source.branch_id={$branch_id};";
            $script .= "ctx._source.time_local='{$time_local}';";
            $script .= '}';

            $put_data_str .= json_encode($inex_type_id) . "\n";
            $put_data_str .= json_encode(['script' => [
                "inline"=>$script,"params"=>[$nested_id=>$goods[$nested_id]]]
                ]) . "\n";
            $save_type_list[$goods['branch_id']] = $index_type;
        }
        ...
        $curl = new \Neigou\Curl();
        $url = config('neigou.ESSEARCH_HOST') . ':' . config('neigou.ESSEARCH_PORT') . '/_bulk';
        return $curl->Post($url, $put_data_str);
    }
    ...
    public function ElasticSearchsSave($save_data)
    {
        ...
        //\Neigou\Logger::Debug('es.update.dml', array('string' => $put_data_str,));
        try{
            //不存在的type
            $null_type_list = array_diff($save_type_list, $use_type_list);
            if (!empty($null_type_list)) {
            ...
            $url = config('neigou.ESSEARCH_HOST') . ':' . config('neigou.ESSEARCH_PORT') . '/_bulk';
            $res = $curl->Post($url, $put_data_str);
            \Neigou\Logger::Debug('es.update.res', array('string' => $res,));
            return $res;
        }catch (\Exception $e){
            \Neigou\Logger::Debug('es.update.res.error', array($e->getMessage(),$e->getFile(),$e->getLine()));
        }

    }

    public function GetElasticSearchMaxGoodsId()
    {
        $array = array(
            "aggs" => array(
                "max_id" => array(
        ...
        $curl = new \Neigou\Curl();
        $url = config('neigou.ESSEARCH_HOST') . ':' . config('neigou.ESSEARCH_PORT').'/'. config('neigou.ESSEARCH_INDEX').'/'.config('neigou.ESSEARCH_TYPE') . '/_search';
        $res = $curl->Post($url,$string);
        $ret = json_decode($res,true);
        $goods_id = $ret['aggregations']['max_id']['value'];
//        dump($res,$ret,$goods_id);die;
        \Neigou\Logger::General('es.get.max.goods', array($string,$url,$goods_id,));
        return $goods_id ?? 0;
    }
