...
class Resource
{
     ...
     * @return  array
     */
    public function getResourceList($object, $objectItem = NULL, $page = 1, $pageSize = 20, $filter = array())
    {
        if (empty($object))
        {
            return array();
        ...
        return  $resourceModel->getResourceList($object, $objectItem, $pageSize, $offset, $filter);
    }
     ...
     * @param       $systemCode     string      系统
     * @param       $errMsg         string      错误信息
     * @return      mixed
     */
    public function lockResource($object, $objectItem, $resource, $resourceItem = '', $resourceValue = '', $memo = '',  $systemCode = 'neigou', & $errMsg = '')
    {
        ...
        // 状态(1:锁定 2:释放 3:消耗)
        $status = 1;
        $resId = $resourceModel->addResource($object, $objectItem, $resource, $resourceItem, $resourceValue, $status, $memo, $systemCode);
