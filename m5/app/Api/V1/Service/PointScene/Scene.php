...
class Scene
{
    ...
    public function Create($sceneData)
    {
        ...
        $saveData = array(
            ...
            "extend_data" => $sceneData['extend_data'] ? $sceneData['extend_data'] : "",
        );
    ...
    public function Update($sceneId, $sceneData)
    {
        ...
        if(!empty($sceneData['extend_data'])){
            $saveData['extend_data'] = $sceneData['extend_data'];
        }
    ...
    public function RelationCompany($relationData)
    {
        ...
        foreach ($relationData['company_list'] as $companyId) {
            $adapterInfo = SceneAdapterCompany::FindAdapterInfoByCompanyId($companyId);
            if(!$adapterInfo){
                app('db')->rollBack();
                return $this->Response(false, "关联失败,公司比例获取失败");
            }

            $accountInfo = array(
                "scene_id"   => $relationData['scene_id'],
                "company_id" => $companyId,
                "exchange_rate" => $adapterInfo->exchange_rate
            );
            $rel_res = SceneCompanyRelModel::FindByCompanyAndScene($companyId,$relationData['scene_id']);
            if($rel_res->id >0){
                continue;
            }
