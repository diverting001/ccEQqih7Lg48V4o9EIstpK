...
class CompanyAccount extends Account
{
    ...
    public function Frozen($frozenData)
    {
        ...
        $memoOperator     = $frozenData['memo_operator'] ?: "";
        ...
        foreach ($frozenAccountList as $accountId => $frozenInfo) {
            ...
            $status = CompanyBusinessRecord::Create([
    ...
    public function AssignToMembers($assignData)
    {
        ...
        foreach ($memberList as $pointData) {
            ...
            $status = MemberBusinessRecord::Create(array(
                ...
                'point'             => $pointNum,
                'after_point'       => $memberPointInfo->point + $pointNum,
                'memo'              => $memo,
                'created_at'        => time()
            ));
            if (!$status) {
        ...
        if ( ! empty($falseMemberList)) {
            $businessFlowServer->Delete($businessFlowCode);
            app('db')->rollBack();
            return $this->Response(false, "发放失败", array('member_list_failed' => $falseMemberList));
        }
        app('db')->commit();
    ...
    public function Income($incomeData)
    {
        ...
        $status = CompanyBusinessRecord::Create(array(
            ...
            'after_point'        => $accountInfo->point + $incomeData['point'],
        ...
        $bindBillRes = $businessFlowServer->BindBillCode(
            $bfCreateRes['data']['business_flow_code'],
    ...
    public function ReleaseFrozen($frozenData)
    {
        ...
        foreach ($returnRelease as $account => $releaseData) {
            ...
            $accountPointInfo = $accountServer->GetAccountInfo($account);
            ...
            $status = CompanyBusinessRecord::Create(array(
                ...
                'record_type'        => 'add',
                'before_point'       => $accountPointInfo->point - $releaseData['point'],
                'point'              => $releaseData['point'],
                'after_point'        => $accountPointInfo->point,
                'memo'               => $memo,
                'created_at'         => time()
            ));
    ...
    public function AssignList($queryData)
    {
        ...
        $businessFlowServer = new BusinessFlowServer();
        $billListRes        = $businessFlowServer->GetBillCodeByBusiness($sourceBusinessType, $sourceBusinessBn,
            $systemCode);
        if (!$billListRes['status']) {
            return $billListRes;
        }
        $billArr = $billToBusinessCode =  array();
        foreach ($billListRes['data'] as $billInfo) {
            $billArr[]                                = $billInfo->bill_code;
            $billToBusinessCode[$billInfo->bill_code] = $billInfo->business_flow_code;
        }
        ...
        $busInfo        = CompanyBusinessRecord::QueryByBusiness($systemCode, $sourceBusinessType, $sourceBusinessBn);
        ...
        foreach ($transferListRes['data'] as $transferInfo) {
            ...
            $returnData[] = array(
                ...
                "business_flow_code"              => $billToBusinessCode[$transferInfo->bill_code] ?? '',
                ...
                'money'                           => $this->point2money($transferInfo->point),
                'member_account_son_point'        => $transferInfo->toaccountson_point,
                'member_account_son_used_point'   => $transferInfo->toaccountson_used_point,
                'member_account_son_frozen_point' => $transferInfo->toaccountson_frozen_point,
                'memo'                            => $transferInfo->memo,
                'memo_operator'                   => $busInfo->memo_operator,
                'created_at'                      => $transferInfo->created_at,
                'overdue_time'                    => $transferInfo->overdue_time,
                'company_overdue_time'            => $transferInfo->accountson_overdue_time,
            );
        }
    ...
    public function CompanyOverduePoint($companyId, $overdueDate, $rowNum, $page)
    {
        $memberIds = SceneMemberRel::QueryOverdueByMemberIdOrCount([
            'companyId' => $companyId,
            'overdueDate' => $overdueDate,
            'pageSize' => $rowNum,
            'page' => $page,
        ]);
        return SceneMemberRel::QueryOverdueByMemberIdsList($companyId, $memberIds, $overdueDate);
    }

    public function CompanyOverduePointCount($companyId, $overdueDate)
    {
        return SceneMemberRel::QueryOverdueByMemberIdOrCount(
            ['companyId' => $companyId, 'overdueDate' => $overdueDate],
            true
        );
    }
}
