...
class Order
{
    ...
    public function GetOrderList($request_data)
    {
        ...
        //获取主订单id
        $main_order_id_list = $this->SelectMainOrderIdList(
            $request_data['filter'],
            $request_data['page_index'],
     ...
     * @todo 筛选出主订单列表
     */
    protected function SelectMainOrderIdList($filter_data, $page_index = 1, $page_size = 10, $orderBy = '')
    {
        $main_order_id_list = [];
        if (empty($filter_data)) {
            return [];
...
//                return $where;
            ...
            switch ($field) {
                ...
                default:
                    if (is_array($value)) {
                        if (array_key_exists('not_in', $value) && is_array($value['not_in'])) {
                            $where[$field] = [
                                'type' => 'not_in',
                                'value' => $value['not_in']
                            ];
            ...
            foreach ($order_items_list as $order_id => $order_item) {
                foreach ($order_item as $bn => $item) {
                    ...
                    if ($item->item_type == 'gift') {
                        if (isset( $format_order_list[$item->order_id]['items'][$item->p_bn])) {
                            $format_order_list[$item->order_id]['items'][$item->p_bn]['gift'][$item->bn] = [
                                'bn' => $item->bn,
                                'name' => $item->name,
                                'cost' => $item->cost,
                                'price' => $item->price,
    ...
    public function GetOrderInfo($request_data)
    {
        ...
        // 计算服务
        $calculate_info = $service_logic->ServiceCall('calculate_log_get', ['order_id' => $order_info->root_pid]);
        $order_info_list = $this->FormatOrderInfoData($order_info, $calculate_info['data']['calculate_result_data']);
        //格式订单数据
        $split_order_list = $this->GetSplitOrdersData([$order_id], $request_data['output_format']);
        //获取拆分订单数据
        if (!empty($split_order_list)) {
            foreach ($split_order_list as $split_order) {
                $order_info_list['split_orders'][$split_order->order_id] = $order_info = $this->FormatOrderInfoData($split_order, $calculate_info['data']['calculate_result_data']);
            }
        }
        return $order_info_list;
    }

    private function FormatOrderInfoData($order_info, $calculate_info = null)
    {
        if (empty($order_info)) {
            return [];
        }
        ...
        if (!empty($order_item_list)) {
            foreach ($order_item_list[$order_info->order_id] as $order_item) {
                ...
                if ($order_item->item_type == 'gift') {
                    if (isset($items[$order_item->p_bn])) {
                        $items[$order_item->p_bn]['gift'][] = [
                ...
                $items[$order_item->bn] = [
                    ...
                    'point_amount' => $order_item->point_amount,
                    'item_type' => $order_item->item_type,
                    'voucher_amount' => isset($calculate_info['product_list'][$order_item->bn]['split_price']['voucher_discount']) ? $calculate_info['product_list'][$order_item->bn]['split_price']['voucher_discount'] : $order_item->pmt_amount,
                    'order_promotion_amount' => $calculate_info['product_list'][$order_item->bn]['split_price']['order_promotion_discount'] ?: 0,
                ];
                $order_info_list['voucher_amount'] = bcadd($order_info_list['voucher_amount'], $items[$order_item->bn]['voucher_amount'], 2);
                $order_info_list['order_promotion_amount'] = bcadd($order_info_list['order_promotion_amount'], $items[$order_item->bn]['order_promotion_amount'], 2);
            }
        }
        $order_info_list['items'] = array_values($items);
        return $order_info_list;
    }
    ...
    private function GetSplitOrdersData($main_order_id_list, $output_format)
    {
        ...
        switch ($output_format) {
            ...
            case 'all_split_order':
                ...
                $split_order_list = OrderModel::GetOrderList('*', $split_where, 40);
                break;
    ...
    public function GetWmsOrderMsgList($filter = [], $page_index = 1, $page_size = 20, $need_total = true)
    {
        ...
        $total = 0;
        $limit = max($page_index - 1, 0) * $page_size . ',' . $page_size;
        $ret_data = OrderModel::GetExceptionOrderMsgListForMis('msg.*', $filter, $limit, $need_total);
        if (!empty($ret_data)) {
            $data = [];
            $data_ids = [];
            foreach ($ret_data['order_list'] as $current_order) {
                $data_ids[] = $current_order->data_id;
            }
            if ($filter['business'] === 'order') {
                $order_ids = $data_ids;
            } else {
                $after_model = new AfterSale();
//                $after_list = $after_model->getList('whereIn', ['key' => 'after_sale_bn', 'value' => $data_ids]);
                $after_sale_bns = '\'' . implode("','", $data_ids) . '\'';
                $after_list_mapping = $after_model->getSearchPageList("after_sale_bn in($after_sale_bns)", 1, $page_size);
                $after_list_mapping = $after_list_mapping['data'];
                $order_ids = array_column($after_list_mapping, 'order_id');
            }
            $where = [
                'order_id' => [
                    'type' => 'in',
            ...
            $order_list_arr = $this->FormatOrderListData($order_list, true);
            foreach ($ret_data['order_list'] as $current_order) {
                $order_id = $filter['business'] === 'order' ? $current_order->data_id : $after_list_mapping[$current_order->data_id]['order_id'];
                $tmp = $order_list_arr[$order_id];
                ...
                if ($filter['business'] === 'after_sale') {
                    $tmp['after_sale_bn'] = $current_order->data_id;
                    $tmp['after_type'] = $after_list_mapping[$current_order->data_id]['after_type'];
                    $tmp['after_status'] = $after_list_mapping[$current_order->data_id]['status'];
                    $tmp['after_create_time'] = $after_list_mapping[$current_order->data_id]['create_time'];
                    $tmp['after_product_data'] = $after_list_mapping[$current_order->data_id]['product_data'];
                }
                $data[] = $tmp;
            }
            $results['list'] = $data;
     ...
     * 获取消息统计
     */
    public function GetMsgStats($company_id = array())
    {
        return OrderModel::GetMsgStats($company_id);
    }
    ...
    public function getCompanyOrderStatistics($search = array(),$column){
        ...
        // 订单类型
        if(!empty($search['system_code'])) {
            $where['system_code'] = [
                'type' => 'eq',
                'value' => $search['system_code'] ,
            ];
        }
        // 查询不同类型的订单
        if(!empty($search['order_category'])) {
            $where['order_category'] = [
                'type' => 'eq',
                'value' => $search['order_category'] ,
            ];
        }
        // 积分渠道查询
        if(!empty($search['point_channel'])) {
            $where['point_channel'] = [
                'type' => 'eq',
                'value' => $search['point_channel'] ,
            ];
        }
     ...
     * @return int
     */
    public function getMemberOrderStatistics($search = array(), $columns)
    {
        ...
        // 公司id
        $where['company_id'] = ['type'  => 'eq', 'value' => $search['company_id']];

        // 不是拆单的商品
        $where['split'] = ['type' => 'eq', 'value' => 1];

        // 订单状态
        if (!empty($search['status'])) {
            if (is_array($search['status'])) {
                $where['status'] = ['type' => 'in', 'value' => $search['status']];
            } else {
                $where['status'] = ['type' => 'eq', 'value' => $search['status']];
            }
        }
        ...
        if (!empty($search['pay_status'])) {
            if(is_array($search['pay_status'])) {
                $where['pay_status'] = ['type' => 'in', 'value' => $search['pay_status']];
            } else {
                $where['pay_status'] = ['type' => 'eq', 'value' => $search['pay_status']];
            }
        }

        // 用户
        if(!empty($search['member_id'])) {
            $where['member_id'] = ['type' => 'eq', 'value' => $search['member_id']];
        }
        ...
        foreach ($columns as $column) {
            $columnsStr .= "SUM(`{$column}`) as {$column}_sum,";
        }
        $columnsStr = trim($columnsStr, ',');

        $sumRes = OrderModel::GetOrderColumnsSum($where, $columnsStr);

        if (!empty($sumRes)) {
            foreach ($sumRes as $key => $res) {
                $sumRes->$key = !empty($res) ? $res : 0;
            }
        }
