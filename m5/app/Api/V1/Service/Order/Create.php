...
class Create
{
    ...
    public function Create($order_data)
    {
        ...
        if ($split_data['error_code'] != 'SUCCESS') {
            //发送消息
            Mq::OrderCreate($order_data['order_id']);
            $funcgion_log('order_get_split_data', false, json_encode(['msg' => '获取拆单服务结果失败', 'split_data' => $split_data]));
        ...
        if (empty($split_info) || empty($split_info['items'])) {
            //发送消息
            Mq::OrderCreate($order_data['order_id']);
            $funcgion_log('empty_split_items', false, json_encode(['msg' => '格式化拆单数据，商品数据为空', 'split_info' => $split_info, 'split_data' => $split_data]));
        ...
        //进行库存锁定
        $response_data = [];
        $stock_lock_response = $this->StockLock($order_data, $split_info, $response_data, $err_msg);
        if (empty($err_msg)) {
            $err_msg = '库存锁定失败';
        }
        if (!$stock_lock_response) {
            ...
            $funcgion_log('product_lock_fail', false, json_encode(['err_msg'=>$err_msg,'response_data'=>$response_data]));
            if (!empty($response_data)) {
                ...
                //发送消息
                Mq::OrderCreate($order_data['order_id']);
                return $this->Response(400, $err_msg);
            }
        }
        //数据保存
        $save_res = $this->Save($order_data, $split_info);
        if (!$save_res) {
            $funcgion_log('order_save_fail', false, json_encode(['msg' => '订单创建失败', 'split_info' => $split_info, 'order_data' => $order_data]));
            //发送消息
            Mq::OrderCreate($order_data['order_id']);
            $this->StockCancelLock($order_data['order_id'], $order_data['channel']);
     ...
     * @todo 货品库存锁定
     */
    private function StockLock($order_data, $split_info, &$response_data, &$err_msg = '')
    {
        ...
        //进行salyut预下单
        $salyut_response = $this->SalyutCreateOrder($order_data, $split_info, $response_data, $err_msg);

        //进行Shop预下单
        $shop_response = $this->ShopCreateOrder($order_data, $split_info, $response_data, $err_msg);
    ...
    public function StockServicceLock($order_data, $split_info, &$response_data)
    {
        ...
        $product_list = [];
        foreach ($split_info['items'] as $item) {
            if (isset($product_list[$item['bn']])) {
                $product_list[$item['bn']]['count'] += $item['nums'];
                continue;
            }
            $product_list[$item['bn']] = [
                'product_bn' => $item['bn'],
                'count' => $item['nums']
            ];
        }
        ...
        if ($stock_response['error_code'] != 'SUCCESS') {
            \Neigou\Logger::General('service_order_create', array(
                'action' => 'stock_temp_lock.miss.params',
                'data'   => $order_data,
                'res'    => $stock_response,
            ));
     ...
     * @todo salyut库存服务锁定
     */
    public function SalyutCreateOrder($order_data, $split_info, &$response_data, &$err_msg)
    {
        ...
        $salyut_response = $salyut_order_logic->Create($order_data, $order_items, $salyut_response_data, $split_info, $err_msg);
        if (!$salyut_response) {
    ...
    * @todo shop库存服务锁定
    */
    public function ShopCreateOrder($order_data, $split_info, &$response_data, &$err_msg)
    {
        $shop_order_logic = new ShopOrder();

        $shop_response = $shop_order_logic->Create($order_data, $split_info, $response_data, $err_msg);
        if (!$shop_response) {
            return false;
        }
        return true;
        ...
        foreach ($splitData['items'] as $item) {
            ...
            if ( ! empty($item['gift'])) {
                foreach ($item['gift'] as $gift_item) {
                    $return['items'][] = [
                        'bn' => $gift_item['product_bn'],
                        'p_bn' => $item['product_bn'],
                        'name' => $gift_item['name'],
                        'cost' => $gift_item['cost'],
                        'price' => $gift_item['price'],
                        'mktprice' => $gift_item['mktprice'],
                        'amount' => $gift_item['amount'],
                        'weight' => $gift_item['weight'],
                        'nums' => $gift_item['nums'],
                        'pmt_amount' => $gift_item['pmt_amount'],
                        'cost_freight' => $gift_item['cost_freight'],
                        'cost_tax' => $gift_item['cost_tax'],
                        'point_amount' => $gift_item['point_amount'],
                        'item_type' => 'gift',
                        'payed_amount' => $this->getPayed($gift_item),
                    ];
                }
            }
        }
