...
class Rule
{
    ...
    public function saveRule($channel, $ruleInfo)
    {
        $ruleChannelModel = new RuleChannelModel();
        $ruleChannelList  = $ruleChannelModel->getRuleChannel();
        if (!$channel || !array_key_exists($channel, $ruleChannelList)) {
        ...
        $oldRuleInfo = $ruleModel->getRuleInfo([
            'channel'     => $channel,
            'external_bn' => $ruleInfo['external_bn']
        ]);

        if ($oldRuleInfo) {
            if ($ruleInfo['name'] != $oldRuleInfo->name || $ruleInfo['desc'] != $oldRuleInfo->desc) {
                $ruleModel->updateRule($oldRuleInfo->id, [
                    'name' => $ruleInfo['name'],
                    'desc' => $ruleInfo['desc'],
                ]);
            }
            $ruleBn = $oldRuleInfo->rule_bn;
        } else {
            $ruleBn = $this->getRuleBn();
