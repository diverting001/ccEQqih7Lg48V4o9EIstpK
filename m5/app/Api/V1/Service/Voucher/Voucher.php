...
class Voucher
{
    ...
    public function getVoucherWithProduct($member_id, $filter)
    {
        ...
        if (empty($voucher_list) && empty($freeshipping_voucher_list) && empty($dutyfree_voucher_list)) {
            return array(
                'voucher_list'      => array(),
...
//        //获取券黑名单
    ...
    /**
     * Notes  : 重写方法 function getVoucherWithProduct，获取用户可用的优惠券，并通过rule规则 & goods属性进行过滤
     * tip    : 实时查询券绑定规则所对应的商品范围，抛弃查表[server_search_business_value]
     * Author : Liuxiaqing
     * Date   : 2024/9/27 2:35 下午
     * @param $member_id
     * @param $company_id
     * @param $filter
     * @return
     * @return array[]|mixed
     */
    public function getVoucherByFilter($member_id,$company_id, $filter){
        $member_model = new VoucherMember();

        // 获取用户[内购券]、[打折券]
        $voucher_list = $member_model->queryMemberBindedVoucherV2($member_id, $company_id,'normal');
        ...
        // 返回的各类券列表数据初始化
        $voucher_return = array(
            'voucher_list' => $voucher_list ?: array(),
            'freeshipping_list' => $freeshipping_voucher_list ?: array(),
            'dutyfree_list' => $dutyfree_voucher_list ?: array(),
        );
        ...
        // 定义好券类型及标识
        $rule_type_flag = array(
            'voucher_list' => 'voucher',
            'freeshipping_list' => 'coupon',
            'dutyfree_list' => 'coupon',
        );

        // 聚合券规则
        $all_rules = array();

        foreach ($voucher_return as $type => $list){
            $type_flag = $rule_type_flag[$type];

            foreach ($list as $voucher){
                $rule_mark = $type_flag . '_' . $voucher->rule_id;

                if (!array_key_exists($rule_mark, $all_rules)){
                    $all_rules[$rule_mark]['rule_condition'] = unserialize($voucher->rule_condition);
                }
            }
        }
        ...
        $rule_products = $ruleManager->filterByRule($all_rules, $filter);

        foreach ($voucher_return as $type => $list){
            $voucher_return[$type] = $this->filterVoucherByRule($list, $rule_products, $rule_type_flag[$type], $filter['products']);
        }

        return $voucher_return;
    }

    private function filterVoucherByRule($voucher_list, $rule_products, $type, $products){
        foreach ($voucher_list as $idx => $voucher){
            $match_rule_data = $rule_products[$type . '_' . $voucher->rule_id];

            if (!empty($match_rule_data) && !empty($match_rule_data['match_products']['product_bn_list'])){
                $match_goods_ids = array();

                foreach ($products as $product){
                    if (in_array($product['bn'], $match_rule_data['match_products']['product_bn_list'])){
                        $match_goods_ids[] = $product['goods_id'];
                    }
                }

                $voucher_list[$idx]->match_goods = array_unique($match_goods_ids);
                $voucher_list[$idx]->limit_cost = $match_rule_data['rule_condition'][0]['filter_rule']['limit_cost'];

                unset($voucher_list[$idx]->rule_condition);
                unset($voucher_list[$idx]->last_modified);
                unset($voucher_list[$idx]->create_id);

                continue;
            }

            unset($voucher_list[$idx]);
        }
