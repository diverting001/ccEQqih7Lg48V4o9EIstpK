...
//            ['status'   ,'=', 1]
    ...
    public function GetStock($product_list, $filter, $product_num_list = array())
    {
        ...
        $thirdProductBn = $this->GetThirdProductBn($product_list);

        if ( ! empty($thirdProductBn)) {
            foreach ($thirdProductBn as $source => $productBns) {
                $class_name = 'App\\Api\\Logic\\StockSource\\' . ucfirst(strtolower($source));
                $class_obj = new $class_name;
                if (method_exists($class_obj, 'getProductStock')) {
                    $stock_result = $class_obj->getProductStock($productBns, $filter, $product_num_list);
                    foreach ($stock_result as $bn => $v) {
                        $stock = isset($v['stock']) ? intval($v['stock']) : 0;
                        $product_stock_list[$bn]['stock'] = $stock;
                        //检查是否可购买状态
                        if (!empty($v['stock_state'])) {
                            $product_stock_list[$bn]['stock_state'] = $v['stock_state'];
                        }
                    }
                }
            }
    ...
    // 获取库存地址
    private function GetThirdProductBn(array $product_bn)
    {
        $return = [];
        if (empty($product_bn)) {
            return $return;
        ...
        foreach ($product_list as $product) {
            if ($product->type == 3) {
                $return[$product->source][] = $product->product_bn;
            }
        }
        return $return;
    }
