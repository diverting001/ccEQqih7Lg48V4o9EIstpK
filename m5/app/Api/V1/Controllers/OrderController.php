...
//        $split_id   = 1;
        ...
        $create_order_data = array(
            ...
            //是否强制使用特殊运费
            'is_use_special_freight' => !empty($order_data['is_use_special_freight']) ? $order_data['is_use_special_freight'] : 0,
            //公司id
    ...
    public function GetOrderListForMis(Request $request)
    {
        ...
        if (!empty($where_data['member_id'])) {
            $filter['member_id'] = $where_data['member_id'];
        }
    ...
    public function GetWmsOrderMsgList(Request $request)
    {
        ...
        if (isset($where_data['id'])) {
            $filter['id'] = $where_data['id'];
        }
        if (isset($where_data['data_id'])) {
            $filter['data_id'] = $where_data['data_id'];
        }
        ...
        if (isset($where_data['member_id'])) {
            $filter['member_id'] = $where_data['member_id'];
        }
        if (isset($where_data['business'])) {
            $filter['business'] = $where_data['business'];
        }
        if (isset($where_data['pop_owner_id'])) {
            $filter['pop_owner_id'] = $where_data['pop_owner_id'];
        }
        if (isset($where_data['company_id'])) {
            $filter['company_id'] = $where_data['company_id'];
        }
        if (isset($where_data['min_data_create_time'])) {
            $filter['min_data_create_time'] = $where_data['min_data_create_time'];
        }
        if (isset($where_data['max_data_create_time'])) {
            $filter['max_data_create_time'] = $where_data['max_data_create_time'];
        }
        if (!empty($where_data['unsearch_msg_type'])) {
            $filter['unsearch_msg_type'] = $where_data['unsearch_msg_type'];
        }
        ...
        // 打印日志
        \Neigou\Logger::Debug("order.GetWmsOrderMsgList", array('action' => 'GetWmsOrderMsgList', 'filter' => $filter, "request" => $where_data, 'results' => $results));

        if (!empty($results)) {
            $order_list = $results['list'];
            $total = $results['total'];
        }
     ...
     * 获取消息统计
     */
    public function GetMsgStats(Request $request)
    {
        $where_data = $this->getContentArray($request);
        $order_service = new OrderOrder();
        $result = $order_service->GetMsgStats($where_data);
    ...
    public function GetOrderList(Request $request)
    {
        ...
        if (!empty($where_data['system_code'])) {
            $system_code_str = trim($where_data['system_code']);
            $system_code = explode(',', $system_code_str);
            if (is_array($system_code)) {
                $where['system_code'] = $system_code;
            } else {
                $where['system_code'] = $system_code_str;
			}
        }
        //收货人手机号
        ...
        //订单状态
        if (!empty($where_data['status']) ) {
            $where['status'] = is_array($where_data['status']) ?  $where_data['status'] : intval($where_data['status']);
        }
        if (!empty($where_data['pay_status'])) {
            $where['pay_status'] = is_array($where_data['pay_status']) ? $where_data['pay_status'] : intval($where_data['pay_status']);
        }
        ...
        if (!empty($where_data['extend_info_code'])) {
            $where['extend_info_code'] = $where_data['extend_info_code'];
        }
        ...
        // 履约平台发货单号
        if (!empty($where_data['wms_delivery_bn'])) {
            $where['wms_delivery_bn'] = $where_data['wms_delivery_bn'];
        }
        // 履约平台编码
        if (!empty($where_data['wms_code'])) {
            $where['wms_code'] = $where_data['wms_code'];
        }
        if(!empty($where_data['order_category'])) {
            $where['order_category'] =$where_data['order_category'];
        }
        if(!empty($where_data['point_channel'])) {
            $where['point_channel'] =$where_data['point_channel'];
        }
        if (!empty($where_data['cur_money'])) {
            $where['cur_money'] = $where_data['cur_money'];
        }
        if (!empty($where_data['point_amount'])) {
            $where['point_amount'] = $where_data['point_amount'];
        }
    ...
    public function OrderConfirm(Request $request)
    {
        ...
        $confirm_order_data = [
            'order_id' => $order_data['order_id'],
            'confirm_data' => $order_data['confirm_data'],
        ];
    ...
    public function UpdateWmsOrderMsg(Request $request)
    {
        ...
        if (!isset($pars['business'])) {
            $pars['business'] = 'order';
        }
        if ($pars['business'] === 'order') {
            $order_data = $pars;
            if (
                empty($pars['data_id']) &&
                (empty($order_data['wms_code']) || (empty($order_data['wms_order_bn']) && empty($order_data['wms_delivery_bn'])))
            ) {
                $this->setErrorMsg('参数错误');
                return $this->outputFormat([], 400);
            }
            $order_service = new OrderOrder();
            if (isset($pars['data_id'])) {
                $order_info = $order_service->GetOrderInfo([
                    'filter' => [
                        'order_id' => $pars['data_id']
                    ]
                ]);
            } elseif (isset($order_data['wms_delivery_bn'])) {
                $request_data['filter'] = array(
                    'wms_delivery_bn' => $order_data['wms_delivery_bn'],
                    'wms_code' => $order_data['wms_code']
                );
    ...
    public function Update(Request $request)
    {
        ...
        // 更新前的地址
        $order_info =  OrderModel::GetOrderInfoById($req_data['order_id']);
        // 监控地址是否变更
        $is_address_change = false ;
        foreach ($update_data as $key=>$value) {
            if(in_array($key ,['ship_mobile' ,'ship_name' ,'ship_province','ship_city','ship_county','ship_town' ,'ship_addr'])) {
                if ($value != $order_info->$key) {
                    $is_address_change = true;
                }
            }
        }
        if(empty($order_info)) {
            $this->setErrorMsg('订单号不存在');
            return $this->outputFormat([], 401);
        }
        if (!empty($update_data)) {
            $order_service = new OrderOrder();
            $result = $order_service->UpdateById($req_data['order_id'], $update_data);
            if(!empty($result) &&  $is_address_change == true )  {
                // 推送队列  地址变更 推送地址到其他系统
                $mq = new \Neigou\AMQP();
                $mq->PublishMessage('service','member.address.pushorder', array(
                    'routing_key'=>'member.address.pushorder',
                    'data'=>array(
                        "order_id" => $req_data['order_id'] ,
    ...
    public function getCompanyOrderStatistics(Request $request){
        ...
        if($requestData['member_id']) {
            $search['member_id'] = $requestData['member_id'];
        }
        ...
        if($requestData['system_code']) {
            $search['system_code'] = $requestData['system_code'] ;
        }
        if($requestData['order_category']) {
            $search['order_category'] = $requestData['order_category'] ;
        }
        ...
        if($requestData['point_channel']) {
            $search['point_channel'] = $requestData['point_channel'];
        }
        /** 设置搜索参数 -- end */
     ...
     * @return array  $search
     */
    private function requestOrderStatistics($requestData, &$errMsg = '')
    {
        $search = array();

        // 参数检查
        if (empty($requestData['company_id'])) {
            $errMsg = '参数错误 : company_id不能为空';
            return false;
        }
     ...
     * @return array
     */
    public function getMemberOrderStatistics(Request $request)
    {
        $requestData = $this->getContentArray($request);

        $errMsg = '';
        $search = $this->requestOrderStatistics($requestData, $errMsg);
        if (!$search) {
        ...
        // 检索字段
        $columnArr = ['cur_money', 'point_amount'];
        ...
        // 获取订单数据
        $orderService = new OrderOrder();
        $orderRes = $orderService->getMemberOrderStatistics($search, $columns);
        if (!$orderRes) {
            $this->setErrorMsg('参数有误');
            return $this->outputFormat([], 400);
        }
    ...
    /**
     * Notes:通过订单id记录异常订单数据
     * User: mazhenkang
     * Date: 2023/7/4 13:44
     * @param Request $request
     * @return array
     */
    public function UpdateWmsOrderMsgByOrderId(Request $request)
    {
        $pars = $this->getContentArray($request);
        if (empty($pars['order_id'])) {
            $this->setErrorMsg('参数错误');
            return $this->outputFormat([], 400);
        }
        ...
        $update_data = [
            ...
            'data_id'          => $order_info['order_id'],
            'wms_code'         => $order_info['wms_code'],
            'type'             => isset($pars['type']) ? $pars['type'] : '1',
            'data_create_time' => $order_info['create_time'],
            'pop_owner_id'     => $order_info['pop_owner_id'],
            'company_id'       => $order_info['company_id'],
        ];
