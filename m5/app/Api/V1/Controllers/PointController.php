...
class PointController extends BaseController
{
        ...
        if (empty($company_point_channel_list)) {
            ...
            foreach ($company_point_channel_list as $point_channel) {
                ...
                $data[] = [
                    ...
                    'point_detailed_addr' => $point_channel->point_detailed_addr,
                ];
            }
     ...
     * @return mixed
     */
    public function GetMemberAllCompanyPoint(Request $request)
    {
        $member_data = $this->getContentArray($request);
        if (empty($member_data['member_id']) || empty($member_data['company_info'])) {
            $this->setErrorMsg('参数错误');
            return $this->outputFormat([], 400);
        }
        $point_service = new PointService();
        $pointDataRes = [];
        foreach ($member_data['company_info'] as $info) {
            $data = [
            ...
            $point_data = $point_service->GetMemberPoint($data);
            if(!$point_data || $point_data['Result'] != 'true'){
                continue;
            }
            $pointDataRes[$info['company_id']] = $point_data['Data'];
        }
     ...
     * @return mixed
     */
    public function GetCompanyChannelByCompanyIds(Request $request)
    {
        $company_data = $this->getContentArray($request);
        if (empty($company_data['company_id'])) {
            $this->setErrorMsg('请指定公司');
        ...
        $is_company_name = $company_data['is_company_name']== 1;//传递了就查积分名字，没传递就不查
        $is_not_only = $company_data['is_not_only']== 1;//传递了就返回当前公司的全部的积分类型，没传递就只获取第一个积分渠道信息

        $data = array();
        $company_point_channel_list = PointModel::GetCompanyChannelByCompanyIds($company_data['company_id'],$is_not_only);
        if (empty($company_point_channel_list)) {
            $this->setErrorMsg('不支持积分支付');
            return $this->outputFormat([], 0);
        } else {
            foreach ($company_point_channel_list as $point_channel) {
                ...
                $data[] = [
                    'company_id' => $point_channel->company_id,
                    'channel' => $point_channel->channel,
                    'point_name' => !empty($point_name) ? $point_name : PointService::DEFAULT_POINT_NAME,
                    'point_type' => $point_channel->point_type,//积分类型
                    'exchange_rate' => $point_channel->exchange_rate,
                    'point_version' => $point_channel->point_version ?? 1,
                    'point_detailed_addr' => $point_channel->point_detailed_addr,
                ];
            }
            $this->setErrorMsg('请求成功');
