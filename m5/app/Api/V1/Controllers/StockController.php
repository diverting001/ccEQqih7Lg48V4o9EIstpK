...
class StockController extends BaseController
{
    ...
    public function TempLock(Request $request)
    {
        ...
        if (empty($product_list) || empty($channel) || empty($lock_obj) || empty($lock_type)) {
            \Neigou\Logger::Debug('service.create.order', array(
                'action' => 'TempLock.miss.params',
                'data'   => $content_data,
            ));
        ...
        foreach ($product_list as $item){
            $bn = $item['product_bn'];
            if (isset($products_bn_arr[$bn])){
                $products_bn_arr[$bn]['count'] += $item['count'];
            }else{
                $products_bn_arr[$bn] = $item;
            }
        }

        $products_bn = array_values($products_bn_arr);
