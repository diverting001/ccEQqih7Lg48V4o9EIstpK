...
use App\Api\Logic\Promotion\Promotion as PromotionLogic;
...
//            $goods[$key]['auto_promotion'] = $auto;
    ...
    public function LockStock(Request $request)
    {
        ...
        $productBns = array_column($post_data['product'], 'bn');
        $goodsLogic = new GoodsLogic();
        $productList = $goodsLogic->GetProductInfo($productBns);
        $goodsRelation = array();
        foreach ($productList as $item){
            $goodsRelation[$item['product_bn']] = $item['goods_id'];
        }
        ...
        foreach ($post_data['product'] as $key => $val) {
            ...
            $val['goods_id'] = $goodsRelation[$val['bn']];
    ...
    public function PayedCancelUnLockStock(Request $request)
    {
        $post_data = $this->getContentArray($request);
        $msg = '';
        $res = PromotionLogic::returnPayedCancelOrderStock($post_data['order_id'], $msg);
        if ($res) {
            $this->setErrorMsg('请求成功');
            return $this->outputFormat($res);
        }
    ...
    public function AfterSaleUnLockStock(Request $request)
    {
        $post_data =  $this->getContentArray($request);
        $res = PromotionLogic::returnAfterSaleStock($post_data['after_sale_bn']);
        if ($res) {
            $this->setErrorMsg('请求成功');
            return $this->outputFormat($res);
        }
        $this->setErrorMsg('售后单解锁失败');
        return $this->outputFormat($res, 505);
    }
}
