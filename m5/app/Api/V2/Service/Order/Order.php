...
namespace App\Api\V2\Service\Order;
...
class Order
{
    ...
    public function GetOrderList($request_data)
    {
        ...
        //未拆单订单
        if ($request_data['filter']['is_use_split']) {
            $order_list = $this->SelectUseSplitOrderList(
                $request_data['filter'],
    ...
    public function GetOrderTotal($filter_data)
    {
        ...
        if ($filter_data['is_use_split']) {
            $where['split'] = [
                'type' => is_array($filter_data['split']) ? 'in' : 'eq',
                'value' => $filter_data['split']
            ];
        } else {
            $where['create_source'] = [
                'type' => 'eq',
     ...
     * @todo 筛选出订单列表
     */
    protected function SelectUseSplitOrderList($filter_data, $page_index = 1, $page_size = 10, $orderBy = [])
    {
        $order_list = [];
        if (empty($filter_data)) {
            return [];
        ...
        $order_list = OrderModel::GetOrderList('*', $where, $limit, $orderByStr);
        if (empty($order_list)) {
            return [];
        }

        $order_list = $this->FormatOrderListData($order_list);
    ...
    protected function SelectMainOrderIdList($filter_data, $page_index = 1, $page_size = 10, $orderBy = [])
    {
        ...
        $order_list = OrderModel::GetOrderList('order_id', $where, $limit, $orderByStr);
        if (empty($order_list)) {
            return $main_order_id_list;
        }
...
//                return $where;
     ...
     * @todo 组织订单列表数据
     */
    public function GenerateOrderList($main_order_id_list, $output_format)
    {
        $order_list = [];
        if (empty($main_order_id_list)) {
            return $order_list;
     ...
     * @todo 订单详情
     */
    public function BatchGetOrderInfo($filter_data)
    {
        $order_list = [];
        if (empty($filter_data)) {
            return $order_list;
        ...
        //订单筛选
        $db_order_list = OrderModel::GetOrderList('*', $where, 0);
        if (empty($db_order_list)) {
            return $order_list;
        }
        foreach ($db_order_list as $v){
            //格式化数据
            $order_list[]   = $this->FormatOrderInfoData($v);
        }
        return $order_list;
    }
}
