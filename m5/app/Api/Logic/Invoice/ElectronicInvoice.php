...
class ElectronicInvoice
{
    ...
    public function SubmitInvoiceInfo($data = array(),$isRetry = false)
    {
        ...
        if ($thirdRes['status'] == true && $isRetry){
            $einvoiceModel->updateEInvoice(array('status' => 0,'frequency' => 0),array('swno' => $data['InvoInfo']['swno']));
        }
    ...
    {
        $aliFile = base_path('/vendor/aliyun/aliyun-oss-php-sdk-2.3.0.phar');
        if (!file_exists($aliFile)) return false;
        require_once "phar://{$aliFile}/autoload.php";
