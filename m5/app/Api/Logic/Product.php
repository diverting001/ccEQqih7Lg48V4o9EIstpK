...
use App\Api\Model\Goods\Product as ProductModel;
...
//        LEFT JOIN sdb_image_image image ON goods.image_default_id=image.image_id
    ...
    public function Get($pars)
    {
        ...
        if (isset($pars['filter']['product_id'])) {
            ...
            $where .= 'products.bn=\'' . addslashes($pars['filter']['product_bn']) . '\'';
        }
        if ($where == '') {
            return [];
        }
        ...
        //如果货品没有图片则将商品给货品
        if(!$return_data['product_images']){
            $return_data['product_images'] = $this->productDefaultImage($return_data['images']);
        }
    ...
    public function getImagesList(&$goods_logic, $image_ids = '')
    {
        ...
        $images_data = [];
        $images_data_obj = $this->_db->table('sdb_image_image')->select('image_id','l_url', 'm_url', 's_url')->whereIn('image_id', $image_ids)->get()->toArray();
        foreach ($images_data_obj as $key => $obj) {
            $images_data[$obj->image_id] = array(
                'l_url'=>$obj->l_url,
                'm_url'=>$obj->m_url,
                's_url'=>$obj->s_url,
            );
        ...
        foreach ($image_ids as $image_id){
            if(isset($images_data[$image_id]) && $images_data[$image_id]){
                $image_array = $images_data[$image_id];
                $goods_logic->fixImg($image_array);
                $return[] = $image_array;
            }
        }
    ...
    // 获取货品最新成本价
    public function GetLatestCostPrice($param) {
        $return = array();
        if (empty($param)) {
            return $return;
        }

        foreach ($param as $plat => $products) {
            switch (strtoupper($plat)) {
                case 'SALYUT':
                    $result = $this->_getSalyutProductLatestCostPrice(array('product_bn' => $products));
                    $return = array_merge($return, $result);
                    break;
                case 'SHOP':
                    $result = $this->_getStoreProductLatestCostPrice(array('product_bn' => $products));
                    $return = array_merge($return, $result);
                    break;
                case 'SHOPNG':
                    $result = $this->_getStoreProductLatestCostPrice(array('product_bn' => $products));
    ...
    private function productDefaultImage($goodsImages = array()){

        $return = array();
        foreach ($goodsImages as $goodsImage_item){
            if($goodsImage_item){
                $return[] = array(
                    'l_url'=>$goodsImage_item['l_url'],
                    'm_url'=>$goodsImage_item['m_url'],
                    's_url'=>$goodsImage_item['s_url']
                );
            }
        }
    ...
    public function initSpec(&$product_list)
    {
        ...
        foreach ($product_list as &$product) {
            if (!empty($product['spec_desc'])) {
                ...
                $product['spec_private_value_id'] = $product['spec_desc']['spec_private_value_id']?current($product['spec_desc']['spec_private_value_id']):'';
    ...
    private function initImg(&$product_list, $goods_ids_str)
    {
        ...
        foreach ($product_list as &$product) {
            ...
            if (!empty($product['image_default_id'])) {
                if(isset($goods_id_2_image_list[$product['goods_id']][$product['image_default_id']])){
                    $product['images'][] = $goods_id_2_image_list[$product['goods_id']][$product['image_default_id']];
                }else{
                    $null_image_list = $goods_logic->GetImages(array($product['image_default_id']));
                    $product['images'][] = $null_image_list[$product['image_default_id']];
                }
            }
    ...
    // 获取Salyut的货品最新成本价
    private function _getSalyutProductLatestCostPrice($productBns)
    {
        $request_params = array(
            'class_obj' => 'SalyutGoods',
            'method'    => 'getProductLatestCostPrice',
            'data'      => json_encode($productBns)
        );
        ...
        \Neigou\Logger::General('service_salyut_product_cost_price', array('sender' => $request_params, 'remark' => $result));
    ...
    // 获取shop的货品最新成本价
    private function _getShopProductLatestCostPrice($productBns)
    {
        $return = array();
        $appKey = config('neigou.SHOP_APPKEY');
        $appSecret = config('neigou.SHOP_APPSECRET');

        $http_url = config('neigou.SHOP_DOMIN'). '/Shop/OpenApi/Channel/V1/Goods/Goods/getGoodsLatestPrice';
        $token = \App\Api\Common\Common::GetShopV2Sign($productBns, $appSecret);
        $data = array(
            'appkey' => $appKey,
            'data' => json_encode($productBns),
            'sign' => $token,
        ...
        \Neigou\Logger::General('service_shop_product_cost_price', array('sender' => $data, 'remark' => $result, 'product_bn' => $productBns));
        if ($curl->GetHttpCode() == 200) {
            $result = json_decode($result, true);
            $return = $result['Data'];
        }
    ...
    // 获取 shopng 的货品最新成本价
    private function _getShopngProductLatestCostPrice($productBns)
    {
        $return = array();
        ...
        \Neigou\Logger::General('service_shopng_product_cost_price', array('sender' => $data, 'remark' => $result, 'product_bn' => $productBns));
        if ($curl->GetHttpCode() == 200) {
            $result = json_decode($result, true);
            $return = $result['Data'];
        }
    ...
    // 获取 ec 的货品最新成本价
    private function _getStoreProductLatestCostPrice($productBns)
    {
        $return = array();
        $productModel = new ProductModel();
        $productList = $productModel->GetProductList(array('bn' => array('in', $productBns['product_bn'])), ['bn', 'cost', 'price', 'mktprice']);
