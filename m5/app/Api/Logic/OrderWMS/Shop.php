...
class Shop extends Order
{
    ...
    public function Create($order_data)
    {
        ...
        $result = $this->SendData('V2/Order/Order/createOrder', $order_data);
    ...
    public function CompleteOrderByWmsDeliveryBn($wms_delivery_bn, &$msg = '')
    {
        ...
        $result = $this->SendData('V2/Order/Order/completeOrder', $send_data);
        $msg = $result['ErrorMsg'];
        if ($result['Result'] != 'true') {
            return false;
        } else {
    ...
    private function SendData($path, $send_data)
    {
        ...
        $token = \App\Api\Common\Common::GetShopV2Sign($send_data, config('neigou.SHOP_APPSECRET'));
        ...
        \Neigou\Logger::General('wms_shop_order', array('data' => json_encode($send_data), 'result' => $result_str));
        return $result;
    }
