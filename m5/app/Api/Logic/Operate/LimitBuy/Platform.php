...
{
    // 获取商品限购
    public function getProductBuyLimit($products, $companyId, $memberId)
    {
        $return = array();
        ...
        foreach ($data['rule_list'] as $rule)
        {
            ...
            $extendData = $rule['condition']['extend_data'] ? $rule['condition']['extend_data'] : array();

            if ($rule['condition']['operator_class'] === 'limit_buy')
            {
                $promotionLimitBuy[$rule['id']] = array(
                    'min_buy'   => $extendData['min_buy'],
                    'max_buy'   => $extendData['max_buy'],
                    'limit_type'   => $extendData['limit_type'],
                    'refresh_time' => !empty($extendData['refresh_time']) ? $extendData['refresh_time'] : 'no_refresh',
                    'sort'      => $rule['sort'],
                    'tips'  => !empty($extendData['tips']) ? $extendData['tips'] : '',
                );
            }
        }
            ...
            foreach ($product['promotion_rule'] as $id)
            {
                ...
                if ( ! isset($productSort[$productBn]) OR $promotionLimitBuy[$id]['sort'] > $productSort[$productBn])
                {
                    ...
                    $return[$productBn] = array(
                        'min_buy' => $promotionLimitBuy[$id]['min_buy'],
                        'max_buy' => $promotionLimitBuy[$id]['max_buy'],
                        'limit_type' => $promotionLimitBuy[$id]['limit_type'],
