...
class Brand
{
    ...
    public function searchList(array $pars, $field = 'brand_id, brand_name, disabled, last_modify')
    {
        ...
        if (!empty($pars['filter']['brand_name'])) {
            if (is_array($pars['filter']['brand_name'])) {
                $_model = $_model->whereIn('brand_name', $pars['filter']['brand_name']);
            } else {
                if (is_numeric($pars['filter']['brand_name'])) {
                    $_model = $_model->where(function ($query) use ($pars) {
                        $query->where('brand_name', 'like', '%' . $pars['filter']['brand_name'] . '%')
                            ->orWhere('brand_id', $pars['filter']['brand_name']);
                    });
                } else {
