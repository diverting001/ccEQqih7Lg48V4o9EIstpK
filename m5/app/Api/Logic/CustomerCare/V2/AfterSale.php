...
use App\Api\Model\AfterSale\AfterSale as AfterSaleV1Mdl;
...
class AfterSale
{
    ...
    public function find($params = array())
    {
        ...
        $actualReasonType = $afterSaleMdl::afterSaleActualReasonType();
        $after_sale_data['actual_reason_type_str'] = $actualReasonType[$after_sale_data['actual_reason_type']] ?: '';
...
//            app('db')->rollback();
        ...
        foreach ($params['products_data'] as $product_item) {
            $product[] = array(
                ...
                'create_time' => $time,
                'item_type' => $product_item['item_type'] ? $product_item['item_type'] : 'product', // 商品类型 product,gift
                'p_bn' => $product_item['p_bn'] ? $product_item['p_bn'] : NULL,
            );
        }
...
//            app('db')->rollback();
        ...
        //获取图片地址
        $image_data = $this->getImageList(explode(',',$params['images']));
...
//        app('db')->commit();
    ...
    public function updateProductRefund($after_sale_bn, $refund_data = array())
    {

        if (!$after_sale_bn || !$refund_data) {
            return false;
        }

        $flag = true;
        $productMdl = new AfterSaleProducts();
        foreach ($refund_data as $product_bn => $refund) {

            $where = array(
                'after_sale_bn' => $after_sale_bn,
                'product_bn' => $product_bn
            );
            $result = $productMdl->updateRefund($where, $refund);
     ...
     * @return array
     */
    public function getImageList( $image_ids = array() )
    {
        if ( empty( $image_ids ) || !is_array( $image_ids ) )
        {
            return [];
        }
        $after_sale_model = new AfterSaleV1Mdl();
        $image_data = $after_sale_model->getImage( $image_ids );

        foreach ( $image_data as &$image )
        {
            $image = '//' . PSR_CDN_WEB_DIANDI_CLUB_DOMAIN  . $image;
        }
     ...
     * @param $params
     */
    public function getActualReasonType($params)
    {
        $afterSaleMdl = new AfterSaleModel();
        $actualReasonType = $afterSaleMdl::afterSaleActualReasonType();

        return $actualReasonType;
    }
}
