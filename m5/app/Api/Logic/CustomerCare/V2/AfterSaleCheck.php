...
class AfterSaleCheck
{
     ...
     * @param array $root_order_info
     * @param string $error
     * @return bool
     */
    public function checkCreateLegal($params = array(), $order_info = array(),$root_order_info = array(), &$error = '')
    {
        ...
        foreach ($order_info['items'] as $item) {
            ...
            $key = self::_createUniqueProductBn($item['bn']);
            $product_bn_num[$key] = isset($product_bn_num[$key]) ? $product_bn_num[$key] + $item['nums'] : $item['nums'];
            if ( ! empty($item['gift'])) {
                foreach ($item['gift'] as $v) {
                    $order_product_bn[] = $v['bn'];
                    $key = self::_createUniqueProductBn($v['bn'], 'gift', $item['bn']);
                    $product_bn_num[$key] = isset($product_bn_num[$key]) ? $product_bn_num[$key] + $v['nums'] : $v['nums'];
                }
            }
        }

        //主单商品数量
        $root_product_bn_num = array();
        foreach ( $root_order_info['items'] as $root_item )
        {
            $key = self::_createUniqueProductBn( $root_item['bn'] );
            $root_product_bn_num[$key] = isset( $root_product_bn_num[$key] ) ? $root_product_bn_num[$key] + $root_item['nums'] : $root_item['nums'];
            if ( !empty( $root_item['gift'] ) )
            {
                foreach ( $root_item['gift'] as $v )
                {
                    $key = self::_createUniqueProductBn( $v['bn'], 'gift', $root_item['bn'] );
                    $root_product_bn_num[$key] = isset( $root_product_bn_num[$key] ) ? $root_product_bn_num[$key] + $v['nums'] : $v['nums'];
                }
            }
        }
        ...
        foreach ($params['products_data'] as $product_item) {
            ...
            $key = self::_createUniqueProductBn($product_item['product_bn'], $product_item['item_type'], $product_item['p_bn']);

            //单次提交货品总数量
            if ($product_item['nums'] > $product_bn_num[$key]) {
                $product_num = false;
                break;
            }
            if (empty($apply_product_data[$key])) {
                $apply_product_data[$key] = array(
                    'apply_product_bn' => $product_item['product_bn'],
                    'apply_product_num' => $product_item['nums'],
                    'apply_product_item_type' => $product_item['item_type'],
                    'apply_product_p_bn' => $product_item['p_bn'],
                );
            } else {
                $apply_product_data[$key]['apply_product_num'] += $product_item['nums'];
            }
        }
        ...
        //根订单货品数量检查
        $totalRootProductNumCheck = $this->checkRootProductNum($order_info, $apply_product_data, $product_bn_num,$root_product_bn_num, $error);
     ...
     * @return bool
     */
    private function checkRootProductNum($order_info = array(), $apply_product_data = array(), $order_product_num = array(),$root_product_bn_num = array(), &$error = '')
    {
        ...
        foreach ($after_product_data as $after_sale_bn => &$product_section) {
            foreach ($product_section as &$product_item_section) {
                ...
                $product_item_section['order_id'] = $after_sale_data[$after_sale_bn]['order_id'];
            }
        }
        ...
        foreach ($apply_product_data as $apply_product_key => $apply_item) {
            ...
            foreach ($after_product_data as $after_product) {
                foreach ($after_product as $after_product_item) {
                    $key = self::_createUniqueProductBn($after_product_item['product_bn'], $after_product_item['item_type'], $after_product_item['p_bn']);
                    //在途判断
                    if ($apply_product_key == $key && $after_product_item['order_id'] == $order_info['order_id']) {
                        if (!$unfinished_bn && (
            ...
            //在途或者完成的数量
            $total_product_num_data[$apply_product_key] = $apply_total_num;

            //完成的数量
            $finish_product_num_data[$apply_product_key] = $finish_total_num;
        }
        ...
        //数量判断(子单/主单同时满足)
        $beyond_bn = '';
        foreach ($apply_product_data as $key => $apply_item) {
            $product_bn = $apply_item['apply_product_bn'];
            if ($apply_item['apply_product_num'] + $total_product_num_data[$key] > $order_product_num[$key] && $apply_item['apply_product_num'] + $total_product_num_data[$key] > $root_product_bn_num[$key]) {
        ...
        //换货商品完成数量判断(子单/主单同时满足)
        $finish_bn = '';
        foreach ($apply_product_data as $key => $apply_item) {
            $product_bn = $apply_item['apply_product_bn'];
            if ($apply_item['apply_product_num'] + $finish_product_num_data[$key] > $order_product_num[$key] && $apply_item['apply_product_num'] + $finish_product_num_data[$key] > $root_product_bn_num[$key]) {
...
//        }
    ...
    // 唯一的售后商品编码
    private static function _createUniqueProductBn($productBn, $itemType = 'product', $pBn = '', $split = '_')
    {
