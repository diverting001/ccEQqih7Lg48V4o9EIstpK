...
namespace App\Api\V5\Service\Calculate;
    ...
    private function pointProsess()
    {
        ...
        foreach ($this->productList as $product) {
            foreach ($product['point_account'] as $account) {
                ...
                $withRuleRes[$account]['goods_list'][$product['goods_bn']] = [
    ...
    private function freightProsess()
    {
        ...
        foreach ($this->productList as $product_info) {
            if (!$product_info['is_free_shipping'] && $product_info['ziti'] != 1) {
                if (isset($pars['product_info'][$product_info['product_bn']])) {
                    $pars['product_info'][$product_info['product_bn']]['weight']   += $product_info['total_weight'];
                    $pars['product_info'][$product_info['product_bn']]['subtotal'] += $product_info['amount'];
                    $pars['product_info'][$product_info['product_bn']]['nums']     += $product_info['quantity'];
                } else {
                    $pars['product_info'][$product_info['product_bn']] = array(
            ...
            if (isset($product_info['gift_list']) && is_array($product_info['gift_list'])) {
                foreach ($product_info['gift_list'] as $giftInfo) {
                    if (!$giftInfo['is_free_shipping'] && $giftInfo['ziti'] != 1) {
                        if (isset($pars['product_info'][$giftInfo['product_bn']])) {
                            $pars['product_info'][$giftInfo['product_bn']]['weight']   += $giftInfo['total_weight'];
                            $pars['product_info'][$giftInfo['product_bn']]['subtotal'] += 0;
                            $pars['product_info'][$giftInfo['product_bn']]['nums']     += $giftInfo['quantity'];
                        } else {
                            $pars['product_info'][$giftInfo['product_bn']] = array(
        ...
        $pars['product_info'] = array_values($pars['product_info']);
