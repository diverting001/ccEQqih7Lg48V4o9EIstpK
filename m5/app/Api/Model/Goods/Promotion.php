...
class Promotion
{
    ...
    public function LockStock($data_param = array())
    {
        ...
        $data['goods_id'] = $data_param['goods_id'];
        $data['nums'] = $data_param['nums'];
        $data['amount'] = $data_param['amount'];
        $data['member_id'] = $data_param['member_id'];
        $data['company_id'] = $data_param['company_id'];
    ...
    public function GetSoldByBn($bn = '', $rid = 0)
    {
        ...
        $sum = app('api_db')->table('server_promotion_stock')->where($where)->sum('nums');
        if ($sum > 0) {
            return $sum;
        } else {
            return 0;
     ...
     * @return int
     */
    public function GetTodaySoldByBn($bn = '', $rid = 0, $dailyRefreshStartTime = '00:00')
    {
        $where = [
            ['bn', $bn], ['rule_id', $rid]
        ];
        $where = $this->_parseRefreshTimeForLimitBuy($where,'day', $dailyRefreshStartTime);
        $sum = app('api_db')->table('server_promotion_stock')->where($where)->sum('nums');
        if ($sum > 0) {
            return $sum;
        } else {
    ...
    /**
     * 根据Bn 获取企业员工对应商品已经购买量
     * @param string $bn
     * @param int $rid
     * @param bool $refresh_time
     * @return int
     */
    public function getTodayCompanySoldNums($bn = '', $rid = 0, $company_id = 0)
    {
        $where = [
            ['bn', $bn],
            ['rule_id', $rid],
        ...
        $where = $this->_parseRefreshTimeForLimitBuy($where,'day');
        $sum = app('api_db')->table('server_promotion_stock')->where($where)->sum('nums');
        if ($sum > 0) {
            return $sum;
        } else {
    ...
    public function getSoldByGoodsIdRule($goodsId = '', $rid = 0, $excludeOrderId = 0)
    {
        $where = [
            ['goods_id', $goodsId], ['rule_id', $rid]
        ];
        if ($excludeOrderId > 0) {
    ...
    public function getTodaySoldByGoodsIdRule($goodsId = '', $rid = 0, $excludeOrderId = 0, $dailyRefreshStartTime = '00:00')
    {
        $where = [
            ['goods_id', $goodsId], ['rule_id', $rid]
        ];
    ...
    public function getUseGoodsAmount($memberId, $companyId, $rid = 0, $excludeOrderId = 0)
    {
        $where = [
            ['member_id', $memberId], ['rule_id', $rid],['company_id', $companyId]
        ];
        ...
        return app('api_db')
                ->table('server_promotion_stock')
                ->where($where)
                ->sum('amount');
    }

    public function getDayUseGoodsAmount($memberId, $companyId, $rid = 0, $excludeOrderId = 0, $dailyRefreshStartTime = '00:00')
    {
        $where = [
            ['member_id', $memberId], ['rule_id', $rid],['company_id', $companyId]
        ];
    ...
    private function _parseRefreshTimeForLimitBuy($where,$refresh_time, $dailyRefreshStartTime = '00:00'){
        switch ($refresh_time){
            case 'day':
                $dailyRefreshStartTime = empty($dailyRefreshStartTime) ? '00:00' : $dailyRefreshStartTime;
                $resetTimeParts = explode(':', $dailyRefreshStartTime);
                $timeNow = time();
                $resetTime = strtotime(date('Y-m-d', $timeNow) . ' ' . $dailyRefreshStartTime);
                $resetDay = ($timeNow < $resetTime) ? -1 : 0;

                // 当前时间大于等于每日重置开始生效时间，以新设置生效时间开始计数; 当前时间小于开始生效时间，以新设置生效时间前一天开始计数
                $beginTime=mktime($resetTimeParts[0], $resetTimeParts[1],0,date('m'),date('d') + $resetDay,date('Y'));
                $endTime=mktime($resetTimeParts[0], $resetTimeParts[1],0,date('m'),date('d') + 1 + $resetDay,date('Y'))-1;
     ...
     * @return int
     */
    public function GetSoldByUid($bn = '', $rid = 0, $member_id = 0,$refresh_time = false, $dailyRefreshStartTime = '00:00')
    {
        $where = [
            ['bn', $bn],
            ['rule_id', $rid],
        ...
        $where = $this->_parseRefreshTimeForLimitBuy($where,$refresh_time, $dailyRefreshStartTime);
        $sum = app('api_db')->table('server_promotion_stock')->where($where)->sum('nums');
        if ($sum > 0) {
            return $sum;
        } else {
     ...
     * @return int
     */
    public function getSoldByGoodsIdMemberRule($goodsId = '', $rid = 0, $member_id = 0, $refresh_time = false, $excludeOrderId = 0, $dailyRefreshStartTime = '00:00')
    {
        $where = [
            ['goods_id', $goodsId],
            ['rule_id', $rid],
    ...
    public function getSoldByMemberAndRule($rid, $member_id, $refresh_time = false, $dailyRefreshStartTime = '00:00')
    {
        $where = [
            ['rule_id', $rid],
            ['member_id', $member_id]
