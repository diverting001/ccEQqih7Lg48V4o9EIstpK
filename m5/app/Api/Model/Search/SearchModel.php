...
class SearchModel
{
    ...
    private function getBusinessValue($business_code, $business_field, $goods_id)
    {
        return get_object_vars($this->_db->table($this->table_search_business_value)
            ->where([
                'goods_id' => $goods_id,
        ...
        foreach ($goods as $good) {
            ...
            if ($good['act'] === 'add') { // 新增
                ...
                if (isset($es_good)) {
                    ...
                    if ($res !== false) {
                        $es_fields_goods[$good['business_field']][$goods_id] = $this->getBusinessValue($business_code, $good['business_field'], $goods_id);
                    }
                }
            } elseif ($good['act'] === 'rm') {
                $modify_values = is_array($good['business_value']) ? $good['business_value'] : array($good['business_value']);
                ...
                if (isset($es_good)) {
                    if ($business_code == "outlet_goods" || $es_good['value'] != json_encode($modify_values)) {
                        // 已存在规则id
                        $res = $this->_db->table($this->table_search_business_value)
                            ->where([
                                'id' => $es_good['id']
