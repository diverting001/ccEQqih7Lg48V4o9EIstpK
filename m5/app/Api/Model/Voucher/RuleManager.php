...
//        $create_data_array = json_decode($json_data, true);
            ...
            if ($create_rule) {
                if (empty($save_data_array['rule_id'])) {
                    ...
                    $create_data = array(
                        ...
                        'display_intro' => $save_data_array['display_intro'] ?? '',
                    );
                    //判断name是否已经存在
                    $map['tag'] = $save_data_array['tag'];
    ...
    // 批量规则通过商品条件匹配符合规则的商品
    public function filterByRule($rules_arr, $filter){
        $time = time();

        $ret_black_list = $this->_db->table('promotion_voucher_blacklist_rule')->where([
            ['type', 1],
            ['start_time', '<=', $time],
            ['end_time', '>=', $time]
        ])->get()->all();

        $rule_black_list = array();

        foreach ($ret_black_list as $black_item){
            if (!empty($black_item->rule)){
                $black_rule = json_decode($black_item->rule, true);
                $rule_black_list[$black_rule['bn']] = $black_rule['bn'];
            }
        }

        foreach ($rules_arr as $key => $rule){
            $rule_data = $rule['rule_condition'][0];

            $rule_type = $rule_data['processor'];

            if (empty($rule_type)){
                $rules_arr[$key]['match_products'] = array();
                continue;
            }

            $rule_type_path = 'App\Api\Model\Voucher\Rule\\' . $rule_type;

            $rule_obj = new $rule_type_path;

            $function_name = !empty($filter['version']) ? 'matchV' . intval($filter['version']) : 'match';

            if (!method_exists($rule_obj, $function_name)){
                $rules_arr[$key]['match_products'] = array();
                continue;
            }

            $rule_data['filter_rule']['ret_black_list'] = $rule_black_list;

            $match_data = $rule_obj->$function_name($rule_data['filter_rule'], $filter);

            $rules_arr[$key]['match_products'] = $match_data ?: array();
        }
