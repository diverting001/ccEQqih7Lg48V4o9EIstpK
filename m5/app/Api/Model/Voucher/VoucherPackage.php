...
class VoucherPackage
{
    ...
    public function createPackageRule($create_params)
    {
        ...
        $rule_data = array(
            ...
            'use_limit' => $create_params['use_limit'],
        );
...
//                    $sql = "select name from {$this->table_voucher_rules} where rule_id={$voucher_item['rule_id']}";
    ...
    // 礼包应用
    public function applyVoucherPkg($apply_params, &$create_result_ids)
    {
        ...
        if ($apply_success) {
            $this->_db->commit();
            $create_result_ids = $apply_success['create_result_ids'];
    ...
    public function queryVoucherPkg($query_params)
    {
        ...
        $rule_voucher_data = $this->_db->table($this->table_voucher_package_rule_voucher . ' as vprv')
            ->join('promotion_voucher_rules as pvr', 'vprv.rule_id', '=', 'pvr.rule_id')
            ->select('vprv.*', 'pvr.rule_condition')
            ->where('pkg_rule_id', $query_params['pkg_rule_id'])
            ->get()
            ->all();
