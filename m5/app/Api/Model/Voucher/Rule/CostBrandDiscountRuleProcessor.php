...
class CostBrandDiscountRuleProcessor extends BasicRuleProcessor
{
    ...
    public function match($rule_condition, $filter_data)
    {
        ...
        foreach ($goods_list as $k => $goods) {
            ...
            $brandDiscountId = $this->getBrandDiscountIdByGoodsId($productBn);
            $goods_price = $goods['obj_items']['products'][0]['price']['price'];
            $number = $goods['quantity'];
            if (in_array($brandDiscountId, $rule_condition['brand_discount'])) {
    ...
    public function matchV6($ruleCondition, $filterData)
    {
        ...
        foreach ($filterData['products'] as $product) {
            $productBns[] = $product['bn'];
        }

        // 获取到店兑品牌数据
        $goodsBrand = $this->_getBrandDiscountIdByProductBns($productBns);
        ...
        foreach ($filterData['products'] as $product) {
            ...
            // 检查到店兑品牌条件
            if (isset($goodsBrand[$product['bn']]) && in_array($goodsBrand[$product['bn']], $ruleCondition['brand_discount'])) {
    ...
    private function getBrandDiscountIdByGoodsId($productBn)
    {
        $result = $this->_db->table('sdb_b2c_brand_discount')->where('product_bn', $productBn)->first();
        if ($result) {
            return $result->id;
        }
        return false;
    ...
    /**
     * 通过 productBns 获取到店兑品牌 ID
     *
     * @param   $productBns   array   规则条件
     * @return  array
     */
    private function _getBrandDiscountIdByProductBns($productBns)
    {
        $return = array();
        ...
        $productBns = implode('","', array_filter($productBns));
        $sql = "select id, product_bn from sdb_b2c_brand_discount where product_bn IN (\"{$productBns}\")";
        $result = $this->_db->select($sql);
        if ($result) {
            foreach ($result as $v) {
                $return[$v->product_bn] = $v->id;
            }
        }
