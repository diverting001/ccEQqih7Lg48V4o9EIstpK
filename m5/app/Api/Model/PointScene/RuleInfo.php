...
class RuleInfo
{
    ...
    public static function Query($consumeType, $filterKey, $ruleId, $filterVal, $filterVal1 = null, $filterVal2 = null, $filterVal3 = null, $act = 'in')
    {
        ...
        if ($ruleId) {
            if (is_array($ruleId)) {
                $ruleId = array_values(array_unique($ruleId));
        ...
        if ($filterVal) {
            if (is_array($filterVal)) {
                $filterVal = array_values(array_unique($filterVal));
        ...
        if ($filterVal1) {
            if (is_array($filterVal1)) {
                $filterVal1 = array_values(array_unique($filterVal1));
        ...
        if ($filterVal2) {
            if (is_array($filterVal2)) {
                $filterVal2 = array_values(array_unique($filterVal2));
        ...
        if ($filterVal3) {
            if (is_array($filterVal3)) {
                $filterVal3 = array_values(array_unique($filterVal3));
    ...
    public static function GetByIDs($rule_id_arr)
    {
        if (empty($rule_id_arr)) {
            return [];
        }
        return app('api_db')->table('server_new_point_rule_info')->whereIn(rule_id, $rule_id_arr)->get();
    }
}
