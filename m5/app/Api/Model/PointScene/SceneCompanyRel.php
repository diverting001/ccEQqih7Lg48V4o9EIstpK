...
class SceneCompanyRel
{
    ...
    public static function Create($relData)
    {
        ...
        try {
            ...
            Logger::General('server.scenePoint.scene.create.fail',['data'=>$relData,'message'=>$e->getMessage()]);
            $relId = false;
        }
        return $relId;
    }
     ...
     * 获取公司关联的所有场景
     */
    public static function QueryByCompany($companyId)
    {
        $where = array(
            'rel.company_id' => $companyId,
            'rel.disabled'   => 0
        ...
        return app('api_db')
            ->table('server_new_point_company_scene_rel as rel')
            ->leftJoin('server_new_point_scene as scene', 'rel.scene_id', '=', 'scene.scene_id')
            ->select('rel.company_id', 'rel.scene_id', 'rel.account', 'scene.name as account_name', 'rel.exchange_rate')
            ->where($where)
            ->get();
