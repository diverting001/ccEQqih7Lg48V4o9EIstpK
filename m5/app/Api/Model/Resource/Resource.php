...
class Resource
{
     ...
     * @return      array
     */
    public function getResourceList($object, $objectItem = NULL, $limit = 20, $offset = 0, $filter = array())
    {
        $return = array();
        ...
        {
            $where['system_code'] = $filter['system_code'];
        }

        if (isset($filter['gt_id']))
        {
            $where[] = ['res_id', '>', $filter['gt_id']];
        }

        $result = app('api_db')->table('server_resources')->where($where)->limit($limit)->offset($offset)->OrderBy('res_id', 'ASC')->get();
     ...
     * @return      boolean
     */
    public function addResource($object, $objectItem, $resource, $resourceItem = '', $resourceValue = '', $status = 1, $memo = '', $systemCode = 'neigou')
    {
        if (empty($object) OR empty($objectItem) OR empty($resource))
        {
            return false;
