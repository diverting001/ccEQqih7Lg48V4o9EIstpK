...
class Point
{
    ...
    public static function GetCompanyChannel($company_id)
    {
        ...
        $sql = "select cc.company_id,c.channel,c.point_name,c.exchange_rate,c.point_type,c.point_version,c.point_detailed_addr from server_point_channel_company as cc
     ...
     * @return array
     */
    public static function GetCompanyChannelByCompanyIds(array $company_id, $is_not_only)
    {
        if (empty($company_id)) {
            return;
        }
        $value = implode(',', array_fill(0, count($company_id), '?'));
        $sql = "select cc.company_id,c.channel,c.point_name,c.exchange_rate,c.point_type,c.point_version,c.point_detailed_addr from server_point_channel_company as cc
                  left join server_point_channel as c on c.channel = cc.channel where company_id in (" . $value . ") and cc.status = 1";
        if(!$is_not_only){
            $sql .= ' GROUP BY cc.company_id';
        }
        $point_channel_list = app('api_db')->select($sql, $company_id);
        return $point_channel_list;
    }
