...
class RuleRegionFourLevel
{
    ...
    {
        try {
            $id = app('api_db')->table('server_delivery_limit_rule_region_four_level')->insertGetId($regionInfo);
        } catch (\Exception $e) {
            $id = false;
        }
        return $id;
    ...
    public static function DeleteByRegionId($regionId)
    {
        try {
            $status = app('api_db')
                ->table('server_delivery_limit_rule_region_four_level')
    ...
    {
        $model = app('api_db')->table('server_delivery_limit_rule_region_four_level')->where('region_id', $regionId);
    ...
    public static function matchingRegionListAll($regionId)
    {
        $model = app('api_db')->table('server_delivery_limit_rule_region_four_level')->where('region_id', $regionId);
