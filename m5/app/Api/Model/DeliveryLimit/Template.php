...
class Template
{
    public static function Create($tempInfo)
    {
        ...
        try {
            $id = app('api_db')->table('server_delivery_limit_template')->insertGetId($tempInfo);
        } catch (\Exception $e) {
            $id = false;
        }
        return $id;
    ...
    public static function Update($tempBn, $tempInfo)
    {
        $tempInfo['update_time'] = time();
        try {
            $status = app('api_db')->table('server_delivery_limit_template')
    ...
    public static function Find($templateBn)
    {
        ...
        return app('api_db')
            ->table('server_delivery_limit_template')
            ->where($where)
            ->first();
    }
}
