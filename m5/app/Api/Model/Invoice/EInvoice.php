...
class EInvoice
{
    ...
    public function createEInvoice($data = array(), $type = 1)
    {
        try {
            ...
            //
            if (!$einvoiceId) throw new Exception(json_encode($baseData));
            ...
            if (!$submitId) throw new Exception(json_encode($submitData));
            ...
            foreach ($orderData['Items'] as $product) {
                ...
                if (!$productId) throw new Exception(json_encode($productData));
            }
            $this->_db->commit();
            return true;
        } catch (\Exception $exception) {
            Logger::General('service.einvoice.create', array('remark' => '添加信息失败', 'data' => json_decode($exception->getMessage(), true)));
            $this->_db->rollback();
            return false;
        }
    }
    ...
    public function UpdateEinvoiceAndDownInfo($invoice = array(),$down = array()){
        if (empty($invoice) || empty($down)) return false;
        $this->_db->beginTransaction();
    ...
    public function getSubmitBaseInfo($whereArr = array()){
        if (!$whereArr) return false;
        return $this->_db->table($this->server_einvoice_submit)->select('swno','id','swno','company_name','company_type','cust_tax_no','cust_bank_account','cust_phone','cust_addr as company_addr','cust_email','order_id','yfpdm','yfphm','chyy','thdh')->where($whereArr)->get()->toArray();
    }
