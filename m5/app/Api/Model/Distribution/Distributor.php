...
class Distributor
{
     ...
     * @return  array
     */
    public function getDistributorList($status = 1)
    {
        $return = array();
        $where = ['status' => $status];
        $result = app('api_db')->table('server_distributors')->where($where)->get();
     ...
     * @return  array
     */
    public function getDistributosMessageScope($filter = array())
    {
        $return = array();

        $where = array();
        if ($filter['dt_bn']) {
            $where['dt_bn'] = $filter['dt_bn'];
        }
        if ($filter['status']) {
        ...
        $result = app('api_db')->table('server_distributor_message_scope')->where($where)->get();
