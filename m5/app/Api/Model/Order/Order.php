...
class Order
{
    ...
    public static function SaveOrder($save_data)
    {
        ...
        self::SaveOrderExtendInfo($save_data);
        if (!$res) {
            app('db')->rollBack();
            return false;
        }
        ...
        if (!empty($split_orders) && count($split_orders) > 1) {
            foreach ($split_orders as $split_order) {
                ...
                self::SaveOrderExtendInfo($split_order);
                if (!$res) {
                    app('db')->rollBack();
                    return false;
                }
     ...
     * @return bool
     */
    private static function SaveOrderExtendInfo($order_data)
    {
        $extend_data = json_decode($order_data['extend_data'], true);
        if (empty($extend_data['price']) || !is_array($extend_data['price'])) {
            return true;
        }
        $item_extend_data = array();
        foreach ($extend_data['price'] as $bn => $priceInfo) {
        ...
        if (empty($item_extend_data)) {
            return true;
        }
        return OrderItemExtend::add($item_extend_data);
    }
    ...
    public static function SaveSplitOrders($new_split_order, $cancel_split_order, $update_order)
    {
        ...
        if (!empty($cancel_split_order)) {
            foreach ($cancel_split_order as $cancel_order) {
                $where = [
//                    'status'    => 1,
                    'split' => 1, // 同一笔订单只能拆一次，避免重复拆出子单的情况
    ...
    public static function GetWmsDeliveryOrder($wms_code, $wms_delivery_bn)
    {
        ...
        if (is_array($wms_code)) {
            $wms_code = implode("','", $wms_code);
        }
        $sql = "select * from server_orders where wms_code in('$wms_code') and wms_delivery_bn ='$wms_delivery_bn' ";
        $order_info = app('api_db')->selectOne($sql);
        return $order_info;
    }
     ...
     * @todo 获取订单列表
     */
    public static function GetOrderList($field = '*', $where = [], $limit = 20, $order = 'create_time desc,order_id asc')
    {
    ...
    public static function GetOrderExtendListByCreateTime($condition = [], $offset = 0, $limit = 20, $fields = array('order_id','extend_data','create_time'))
    {
       return app('api_db')
                ->table('server_orders')
                ->select($fields)
                ->whereBetween('create_time',$condition)
                ->offset($offset)
                ->limit($limit)
    ...
    public static function GetOrderExtendCountByCreateTime($condition= [], $fields = array('order_id','extend_data','create_time'))
    {
        return app('api_db')
            ->table('server_orders')
            ->select($fields)
        ...
        foreach ($where as $field => $value) {
            switch ($value['type']) {
                ...
                case 'not_in':
                    $not_in_data = [];
                    foreach ($value['value'] as $k => $data) {
                        $not_in_data[$field . '_' . $k] = "{$data}";
                    }
                    $where_data['str'] .= ' and ( ' . $field . ' not in (:' . implode(',:', array_keys($not_in_data)) . '))';
                    $where_data['bindings'] = array_merge($where_data['bindings'], $not_in_data);
                    break;
                case 'like':
    ...
    */
    ...
    public static function GetTimeoutUncompleteOrders($start_time, $end_time)
    {
        $sql = "select * from `server_orders` where "
            . "`pay_status`=2 "
            . "and `status`=1 "
            . 'and company_id in(2246,2343,16545,16546,16547,16548,20812,30067,37642,37763,42222,43054,43185,43187,43765,34335,47642,47510,46553) '
    ...
    public static function GetTimeoutShipOrders($create_time, $days = 30)
    {
        ...
        $sql = "select o.* from `server_orders` as o left join server_order_items as oi on oi.order_id = o.order_id  where "
            . "`pay_status`=2 "
            //. "and `confirm_status`=2 "
            . "and `status`=1 "
            . "and `ship_status`=1 "
            . "and `split`='1' "
            . 'and company_id in(2246,2343,16545,16546,16547,16548,20812,30067,37642,37763,42222,43054,43185,43187,43765,34335,47642,47510,46553) '
            . "and `pay_time`<=:create_time and `pay_time`>=:begin_time and oi.bn not in('SHOPNG-5EBDFBFDAA149-522') group by o.order_id";
    ...
    public static function GetExceptionOrderMsgListForMis(
        ...
        if (!empty($filter)) {
            $where_str = '1';
            if (isset($filter['id'])) {
                $where_str .= " and msg.`id`=:id";
                $where_data['id'] = $filter['id'];
            }
            if (isset($filter['data_id'])) {
                $where_str .= " and msg.`data_id`=:data_id";
                $where_data['data_id'] = $filter['data_id'];
            }
            if (isset($filter['msg_status'])) {
                $where_str .= " and msg.`status`=:msg_status";
                $where_data['msg_status'] = $filter['msg_status'];
            }
            if (isset($filter['msg_type'])) {
                $where_str .= " and msg.`type`=:msg_type";
                $where_data['msg_type'] = $filter['msg_type'];
            }
            if (isset($filter['business'])) {
                $where_str .= " and msg.`business`=:business";
                $where_data['business'] = $filter['business'];
            }
            if (!empty($filter['pop_owner_id'])) {
                $where_str .= " and msg.`pop_owner_id` in(:pop_owner_id)";
                $where_data['pop_owner_id'] = implode(',', $filter['pop_owner_id']);
            }
            if (!empty($filter['company_id'])) {
                $where_str .= " and msg.`company_id` in(".implode(',', $filter['company_id']).")";
            }
            if (!empty($filter['min_data_create_time'])) {
                $where_str .= " and msg.`data_create_time` >= :min_data_create_time";
                $where_data['min_data_create_time'] = $filter['min_data_create_time'];
            }
            if (!empty($filter['max_data_create_time'])) {
                $where_str .= " and msg.`data_create_time` <= :max_data_create_time";
                $where_data['max_data_create_time'] = $filter['max_data_create_time'];
            }
            if (!empty($filter['unsearch_msg_type'])) {
                $where_str .= ' AND msg.`type` not in(' . implode(',', $filter['unsearch_msg_type']) . ') ';
            }
            ...
            if (isset($filter['msg_wms_order_status'])) {
                if (is_array($filter['msg_wms_order_status'])) {
                    ...
                    $where_str .= " and msg.`wms_order_status` in ({$msg_wms_order_status_vals})";
                } else {
                    $where_str .= " and msg.`wms_order_status`=:msg_wms_order_status";
        ...
        $limit = empty($limit) ? '' : ' limit ' . $limit;
        $sql = "select {$filed} from server_exception_msg as msg where {$where_str} order by {$order} {$limit}";
        if (!empty($filter['member_id'])) {
            $where_str .= ' AND orders.member_id in(' . implode(',', $filter['member_id']) . ')';
            $sql = "SELECT {$filed} from server_exception_msg as msg
    LEFT JOIN server_orders AS orders ON msg.data_id = orders.order_id
    WHERE {$where_str} ORDER BY {$order} {$limit}";
        }
        $results['order_list'] = app('api_db')->select($sql, $where_data);
        if ($need_total) {
            $sql = "select count(1) as total from server_exception_msg as msg where {$where_str}";
            if (!empty($filter['member_id'])) {
                $sql = "SELECT count(1) as total from server_exception_msg as msg
    LEFT JOIN server_orders AS orders ON msg.data_id = orders.order_id
    WHERE {$where_str}";
            }
    ...
    {
        if ($update_data['business'] === 'order') {
            $order_info = self::GetOrderInfoById($update_data['data_id']);
        ...
        $sql = "select * from server_exception_msg where data_id=:data_id and business=:business and type=:type";
        $record = app('api_db')->selectOne($sql, [
            'order_id' => $update_data['order_id'],
            'business' => $update_data['business'],
        ...
        $sql = "insert into server_exception_msg({$keys}) "
            . "values({$vals});";
    ...
    {
        $sql = "update server_exception_msg set `status`=:status,`operator`=:operator,`reason`=:reason, `operator_time`=:operator_time where `id` in (" . $id . ')';
     ...
     * 获取消息统计
     */
    public static function GetMsgStats($filter = array())
    {
        $result = [];

        $sql = "select count(1) as total from server_exception_msg where `status`=0";
        if (isset($filter['company_id']) && !empty($filter['company_id'])) {
            $sql .= " and `company_id` in(".implode(',', $filter['company_id']).")";
        }

        if (isset($filter['unsearch_msg_type']) && !empty($filter['unsearch_msg_type'])) {
            $sql .= " and `type` not in(".implode(',', $filter['unsearch_msg_type']).")";
        }
    ...
    public static function GetInterceptAndHungupSuccessListV2(
        ...
        if (!empty($filter)) {
            ...
            if (!empty($filter['member_id'])) {
                $where_str .= ' and O.`member_id` in(' . implode(',', $filter['member_id']) . ')';
            }
        }
        ...
        if ($need_total) {
            ...
            if (!empty($filter['member_id'])) {
                $sql = "select count(1) as total from server_intercept_orders I join server_orders O on I.order_id=O.order_id where {$where_str}";
            }
            $order_total = app('api_db')->selectOne($sql, $where_data);
            $results['total'] = empty($order_total) ? 0 : $order_total->total;
        }
     ...
     * @return mixed
     */
    public static function GetOrderColumnsSum($where = [], $columns = '')
    {
        if (empty($columns)) {
            return false;
        }
        ...
        $where_data = self::WhereAnalysis($where);
        $sql = "select {$columns} from server_orders where 1 {$where_data['str']}  ";

        return app('api_db')->selectOne($sql, $where_data['bindings']);
    }

    // 根据订单ID获取完整的主+子订单列表
    public static function GetFullOrderList($where = [], $fields = '*')
    {
        $where_data = self::WhereAnalysis($where);
        $sql = "select {$fields} from server_orders where 1 {$where_data['str']}";

        $full_order_list = app('api_db')->select($sql, $where_data['bindings']);
