...
class Calculate
{
    ...
    // 增加新的积分积分使用规则：0（默认）积分可以部分抵扣支付
    const POINT_DEDUCT_TYPE_DEFAULT = 0;
    // 1 积分不可部分抵扣支付
    const POINT_DEDUCT_TYPE_FULL = 1;
    ...
    public function run(
        ...
        //初始化资产
        $this->assetsList['payment_tax_fee_rate']        = $assetsList['payment_tax_fee_rate'] ?? array();
        ...
        // 初始化订单优惠规则
        $this->order_use_rules  = $order_use_rules;
        ...
        foreach ($goodsList as $product) {
            ...
            foreach ($product['gift_list'] as $giftItem) {
                $goodsIdList[]   = $giftItem['goods_id'];
                $goodsBnList[]   = $giftItem['goods_bn'];
                $productBnList[] = $giftItem['product_bn'];
            }
        }
        ...
        $goodsExtList    = $storeDB->table('sdb_b2c_goods as goods')
                                   ...
                                   ->select('goods.bn', 'goods.ziti', 'goods.jifen_pay', 'goods.brand_id', 'goods.brand_id',
                                       'cat.cat_path')
        ...
        $totalTaxfees = 0;
        foreach ($goodsList as $product) {
            if (isset($goodsExtListArr[$product['goods_bn']])) {
                $product = array_merge($product, $goodsExtListArr[$product['goods_bn']]);
            }
            ...
            foreach ($product['gift_list'] as $key => $giftItem) {
                if (isset($goodsExtListArr[$giftItem['goods_bn']])) {
                    $product['gift_list'][$key] = array_merge($giftItem, $goodsExtListArr[$giftItem['goods_bn']]);
                }
                if (in_array($giftItem['product_bn'], $o2oProductBnList) || $giftItem['ziti']) {
                    $product['gift_list'][$key]['is_free_shipping'] = true;
                } else {
                    $product['gift_list'][$key]['is_free_shipping'] = false;
                }
            }
        ...
        //运费计算
        $freightRes = $this->batchFreightProcess();
    ...
    private function pointProsess()
    {
        ...
        // 增加新的积分积分使用规则：0（默认）积分可以部分抵扣支付 1 积分不可部分抵扣支付
        $pointDeductType = isset($this->assetsList['point']['point_deduct_type']) ? $this->assetsList['point']['point_deduct_type'] : self::POINT_DEDUCT_TYPE_DEFAULT;
        foreach ($withRuleRes as $withProductRule) {
            ...
            foreach ($this->productList as $bn => $productInfo) {
                if (array_key_exists($productInfo['product_bn'], $withProductRule['product_list'])) {
                    if ($productInfo['jifen_pay']) {
            ...
            $this->resultData['extend_data']['point'][$sceneId] = array(
                ...
                'point_deduct_type'      => $pointDeductType
            );
        }
        ...
        foreach ($this->assetsList['point']['use_point'] as $sceneId => $curPointData) {
            ...
            // 根据积分使用规则（pointDeductType）来判断,是否允许积分部分抵扣支付
            if ($pointDeductType == self::POINT_DEDUCT_TYPE_FULL){
                // 商品实际金额
                $goodsTotalAmount = $this->resultData['price']['goods_amount'];
                // 积分实际可最多抵扣金额
                $pointDeductAmount = $this->point2money($sceneMaxDiscountPoint, $pointChannel, $sceneId);
                // 订单金额<商品实际金额
                if ($pointDeductAmount < $goodsTotalAmount){
                    $sceneMaxDiscountPoint = 0;
                }
            }
            ...
            foreach ($this->productList as $bn => $productInfo) {
                if ( ! array_key_exists($productInfo['product_bn'], $curSceneRule['product_list'])) {
                    continue;
                }
        ...
        foreach ($this->assetsList['voucher'] as $voucherNumArr => $voucherInfo) {
            ...
            foreach ($voucherInfo['match_product_bn'] as $productBn) {
                ...
                $voucherData['filter_data']['products'][] = [
                    ...
                    'amount' => $productInfo['split_price']['cash_amount'],
                ];
            }
    ...
    private function batchFreightProcess()
    {
        $freightList = $pars = [];
        foreach ($this->productList as $product_info) {
            if ($product_info['single_freight']) {
                if ( ! $product_info['is_free_shipping'] && $product_info['ziti'] != 1) {
                    $freightList[] = [
                        $product_info['product_bn'] = array(
                ...
                if ( ! $product_info['is_free_shipping'] && $product_info['ziti'] != 1) {
                    if (isset($pars[$product_info['product_bn']])) {
                        $pars[$product_info['product_bn']]['weight']   += $product_info['total_weight'];
                        $pars[$product_info['product_bn']]['subtotal'] += $product_info['amount'];
                        $pars[$product_info['product_bn']]['nums']     += $product_info['quantity'];
                    } else {
                        $pars[$product_info['product_bn']] = array(
                            'product_bn' => $product_info['product_bn'],
                            'weight'     => $product_info['total_weight'],
                            'subtotal'   => $product_info['amount'],
                            'nums'       => $product_info['quantity'],
                ...
                //赠品算运费
                if (isset($product_info['gift_list']) && is_array($product_info['gift_list'])) {
                    foreach ($product_info['gift_list'] as $giftInfo) {
                        if ( ! $giftInfo['is_free_shipping'] && $giftInfo['ziti'] != 1) {
                            if (isset($pars[$giftInfo['product_bn']])) {
                                $pars[$giftInfo['product_bn']]['weight']   += $giftInfo['total_weight'];
                                $pars[$giftInfo['product_bn']]['subtotal'] += 0;
                                $pars[$giftInfo['product_bn']]['nums']     += $giftInfo['quantity'];
                            } else {
                                $pars[$giftInfo['product_bn']] = array(
                                    'product_bn' => $giftInfo['product_bn'],
                                    'weight'     => $giftInfo['total_weight'],
                                    'subtotal'   => 0,
                                    'nums'       => $giftInfo['quantity'],
                                );
                            }
                        }
                    }
    ...
    private function freightProsess($item)
    {
        //没有传地址不计算运费
        ...
        $freightList = $this->freightLogic->getProductDeliveryFreight($pars);

        foreach ($freightList as $freight_info) {
            ...
            foreach ($freight_info['product_bn'] as $product_bn) {
                foreach ($pars['product_info'] as $item) {
                    if ($product_bn == $item['product_bn']) {
                        $allQuantity += $item['nums'];
                    }
                }
            }

            $productFreightArr = [];
            foreach ($freight_info['product_bn'] as $freightProdBn) {
                foreach ($this->productList as $productBn => $product) {
                    $productFreight = 0;

                    if ($freightProdBn == $productBn) {
                        $productFreight = $this->number2price(
                            ($product['quantity'] / $allQuantity) * $freight_info['freight'],
                            0,
                            0
                        );
                        $this->productList[$productBn]['freight']['freight_message'] = $freight_info['freight_message'];
                    }


                    foreach ($product['gift_list'] as $giftInfo) {
                        if ($freightProdBn == $giftInfo['product_bn']) {
                            $productFreight += $this->number2price(
                                ($giftInfo['quantity'] / $allQuantity) * $freight_info['freight'],
                                0,
                                0
                            );
                        }
                    ...
                    if (isset($productFreightArr[$productBn])) {
    ...
    private function promotionProsess()
    {
        ...
        $goodsAmount = 0;
        $productList = [];
        foreach ($this->productList as &$calcProductInfo) {
            foreach ($calcProductInfo['gift_list'] as &$giftItem) {
                $giftItem['split_price']['promotion_discount'] = $this->number2price($giftItem['amount']);

                $giftAmount = $this->number2price($giftAmount + $giftItem['amount']);
            }
            $goodsAmount = $goodsAmount + $calcProductInfo['amount'];

            $productList[] = array(
                'name' => $calcProductInfo['product_name'],
        ...
        $this->resultData['price']['goods_amount']      = $this->number2price($goodsAmount + $giftAmount);
        $this->resultData['discount']['promotion']      = 0;
        $this->resultData['discount']['gift_promotion'] = $this->number2price($giftAmount);

        //现在运营服务只管是否免运费,如果没有使用的规则就直接返回成功
        ...
        if (!empty($this->order_use_rules)) {
            $postData['order_use_rule'] = $this->order_use_rules;
        }

//        $promotionRes = $this->serviceLogic->ServiceCall('check_goods_promotion', $postData);
        ...
        $promotionData = [];
        foreach ($promotionRes['data']['product'] as $product) {
            $promotionData[$product['bn']] = $product;
        }
        ...
        foreach ($this->productList as &$calcProductInfo) {
            ...
            foreach ($curProdPromData['use_rule'] as $ruleId) {
                ...
                if ($calcData['status'] == true && $calcData['data']['class'] == 'free_shipping') {
                    $calcProductInfo['is_free_shipping'] = 1;
                }
            }
        }
        if (!empty($this->order_use_rules) && empty($promotionRes['data']['order'][$this->order_use_rules[0]])) {
            return [
                "status" => false,
                "msg" => '运营服务失败',
                "code" => '1012',
            ...
            $order_promotion = $promotionRes['data']['order'][$this->order_use_rules[0]];
            $order_calc = $order_promotion['calc'];
            if ($order_calc['status'] === true) {
                $this->resultData['discount']['order_promotion'] = $order_calc['data']['order_discount'];
                //订单满减金额拆分到products
                $order_discount = $order_calc['data']['order_discount'];
                //券抵扣金额拆分到products
                $product_subtotal_list = array();
                foreach ($order_promotion['products'] as $product_info) {
                    $product_subtotal_list[$product_info['bn']] = $this->productList[$product_info['bn']]['split_price']['cash_amount'];
                }
                // 参与订单优惠的商品总金额
                $product_subtotal = array_sum($product_subtotal_list);
                if ($order_discount > $product_subtotal) {
                    $order_discount = $product_subtotal;
                }
                $this->resultData['discount']['order_promotion'] = $this->number2price($order_discount);
                $product_discount = array();
                foreach ($product_subtotal_list as $bn => $product_price) {
                    $product_order_discount = $this->number2price($product_price / $product_subtotal * $order_discount);
                    $product_discount[$bn] = $product_order_discount;
                    $this->productList[$bn]['split_price']['order_promotion_discount'] = $product_order_discount;
                    $this->productList[$bn]['split_price']['cash_amount'] = $this->number2price($this->productList[$bn]['split_price']['cash_amount'] - $product_order_discount);
                }
                //确保要减去的值不能为0
                asort($product_discount);
                //如果进位后金额与优惠券金额不一致 保留后一位做减法
                if (abs($order_discount - array_sum($product_discount)) > 0.001) {
                    $key = array_last(array_keys($product_discount));
                    $differ = $order_discount - array_sum($product_discount);
                    $this->productList[$key]['split_price']['order_promotion_discount'] = $this->number2price($this->productList[$key]['split_price']['order_promotion_discount'] + $differ);
                    $this->productList[$key]['split_price']['cash_amount'] = $this->number2price($this->productList[$key]['split_price']['cash_amount'] - $differ);
                }
            }
        }
    ...
    private function initProduct($product)
    {
        ...
        $info['point_account']         = $product['point_account'];
        $info['single_freight']        = $product['single_freight'] ? true : false;
        $info['payment_tax_fee_rate']  = $product['payment_tax_fee_rate'] ?? array();
        ...
        $giftList = [];
        foreach ($product['gift_list'] as $giftItem) {
            $giftList[$giftItem['product_bn']] = [
                'shop_id'          => $giftItem['shop_id'],
                'goods_id'         => $giftItem['goods_id'],
                'goods_bn'         => $giftItem['goods_bn'],
                'product_id'       => $giftItem['product_id'],
                'product_bn'       => $giftItem['product_bn'],
                'product_name'     => $giftItem['product_name'],
                'weight'           => $giftItem['weight'],
                'quantity'         => $giftItem['quantity'],
                'total_weight'     => $giftItem['weight'] * $giftItem['quantity'],
                'price'            => $this->number2price($giftItem['price']),
                'amount'           => $this->number2price($giftItem['price'] * $giftItem['quantity']),
                'brand_id'         => $giftItem['brand_id'],
                'cat_path'         => $giftItem['cat_path'],
                'mall_list_id'     => $giftItem['mall_list_id'],
                'is_free_shipping' => $giftItem['is_free_shipping'],
            ];
        }

        $info['gift_list'] = $giftList;
    ...
    public function paymentTaxFee()
    {
        foreach ($this->productList as &$product) {
            ...
            if (!empty($payment_tax_fee_rate['cash_rate'])) {
                //计算现金追加金额
                $tmp_cash = bcmul($product['split_price']['cash_amount'], $payment_tax_fee_rate['cash_rate'], 2);//计算得到应收现金对应的服务费
                $product['payment_tax_fee_rate']['cash']['base_amount'] = $product['split_price']['cash_amount'];//保存这个服务费金额
            ...
            if (!empty($payment_tax_fee_rate['point_rate'])) {
                /**
                 * $this->productList[$bn]['point'] = [ $sceneId => [ 'scene_id'    => $sceneId, 'point'       => 0, 'point_money' => 0, ], ];
                 */
                $all_point_money = 0;
                foreach ($product['point'] as $tmp) {
            ...
            $product['taxfees']['cost_tax'] = bcadd($product['taxfees']['cost_tax'], $tmp_cost_tax, 2);//税费追加
