...
use App\Api\V3\Service\Point\NeigouPoint;
use App\Api\V3\Service\Point\NeigouScenePoint;
...
class PointController extends BaseController
{
    ...
    /**
     * 获取用同一个用户在多个公司的积分账户【批量】
     * @param Request $request
     * @return array
     */
    public function GetMemberAllCompanyPoint(Request $request): array
    {
        $requestData = $this->getContentArray($request);

        if (empty($requestData['company_info']) || empty($requestData['member_id'])) {
            $this->setErrorMsg('参数错误');
            return $this->outputFormat([], 400);
        }
        /** @var Point|NeigouPoint|NeigouScenePoint $pointService */
        $pointService = new Point();
        $pointDataRes = [];
        foreach ($requestData['company_info'] as $info) {
            $res = $pointService->GetMemberPoint([
                'company_id' => (int)$info['company_id'],
                'member_id' => (int)$requestData['member_id'],
                'channel' => $info['channel']
            ]);
            if (!$res['status']) {
                continue;
            }
            $pointDataRes[$info['company_id']] = $res['data'];
        }
        $this->setErrorMsg('获取成功');
        return $this->outputFormat($pointDataRes, 0);
    }
    /**
     * 锁定用户积分
     * @param Request $request
    ...
    public function LockMemberPoint(Request $request)
    {
        ...
        if (empty($recordInfo)) {
            ...
            if($recordInfo->status == 2 ) {
               return $this->outputFormat($recordInfo, 0);
            }
        ...
        $lockDataRes = $pointService->LockMemberPoint([
            ...
            'extend_data'     => $requestData['extend_data'] ?? [],
    ...
    public function RefundMemberPoint(Request $request)
    {
        ...
        if (!empty($refundList)) {
            foreach ($refundList as $refund) {
                if ($refund->status == 2 && $refund->refund_id == $requestData['refund_id']) {
                    $this->setErrorMsg('积分已退还');
        ...
        PointRefund::UpdatePointRefundRecord(
            [
                'status'      => 2,
                'trade_no'    => $refundDataRes['data']['trade_no'] ? $refundDataRes['data']['trade_no'] : '',
                'update_time' => time(),
            ],
            [
                'id'     => $refundId,
     ...
     * @return array
     */
    public function GetRecordDetailByUse(Request $request)
    {
        $requestData = $this->getContentArray($request);

        if (empty($requestData['use_type']) || empty($requestData['use_obj'])) {
            $this->setErrorMsg('参数错误');
            return $this->outputFormat([], 400);
        }
