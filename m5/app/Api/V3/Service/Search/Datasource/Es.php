...
use App\Api\V3\Service\Search\Elasticsearchcreateindex;
...
class Es
{
    ...
    public function ParseFilter($filter_data, $stages = array())
    {
        ...
        //门店ID筛选
        if (isset($filter_data['outlet.outlet_id']) && is_array($filter_data['outlet.outlet_id']) && $filter_data['outlet.outlet_id']) {
            $this->query_data['query']['bool']['filter'][]['nested'] = [
                "path" => "outlet_list",
                "query" => [
                    "bool" => [
                        "must" => [[
                            "terms" => [
                                "outlet_list.outlet_id" => $filter_data['outlet.outlet_id'],
                            ]
                        ]]
                    ]
                ]
