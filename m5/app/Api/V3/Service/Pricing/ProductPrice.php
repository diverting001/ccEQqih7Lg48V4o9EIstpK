...
class ProductPrice {
    ...
    public function GetProductsPrice($product_bn_list){
        ...
        $product_list   = $this->model->select($sql);
        ...
        foreach ($product_list as $k=>$v){
            $good_id = $v['goods_id'] ;
            $goods_id_list[$good_id]  = $good_id;
        }
        $mall_goods_list    = $this->model->GetMallGoodsData($goods_id_list);
        ...
        $goods_id_arr = array_keys($goods_id_list);
        $goods_id_2_cat_ids   = (new Cats())->getCatIds($goods_id_arr);
        //货品数据
        if(!empty($product_list)){
            foreach ($product_list as $k=>&$v){
                $v['mall_list'] = isset($mall_goods_mapping[$v['goods_id']])?$mall_goods_mapping[$v['goods_id']]:array();
                $this->point_price($v);
                $v['cat_ids'] = $goods_id_2_cat_ids[$v['goods_id']];
            }
        }
        //货品所在微仓数据
        $product_branch_mapping = array();
        $branch_product_mode    = $this->model->setTable('sdb_b2c_branch_product')  ;
        $where  = array(
          'product_bn' => $product_bn_list ,
        ) ;
        $branch_product_list    = $branch_product_mode->getBaseInfo($where,['branch_id','product_bn','price','cost','mktprice']);
        ...
        $container_product_list = $this->model->select($sql);
    ...
    private function SetDiscount(){
        if(empty($this->discount)){
            $_discount = (new Goods())->setTable("sdb_b2c_cat_discount") ;
            $this->discount = $_discount->getBaseInfo([],'*');
        }
    }
    ...
    public function GetProductsPriceV2($product_bn_list){
        ...
        $sql = "select p.bn as product_bn,p.price,p.point_price,p.mktprice,g.cat_id,gc.cat_path,g.brand_id,p.goods_id,p.cost from sdb_b2c_products as p
                  ...
                  left join sdb_b2c_goods_cat as gc on gc.cat_id=g.cat_id where p.bn in ({$product_bns})";
        $this->model = new Goods() ;
        $temp_product_list   = $this->model->select($sql);
