...
namespace App\Api\V3\Service\PointServer;
...
class Account
{
    ...
    public function GetAccountInfo($account)
    {
        ...
        $sonAccountGroupList = SonAccountModel::GetAccountTotalValidPoint([$accountInfo->account_id], time());

        $accountId = $accountInfo->account_id;

        $sonAccountTotal = $sonAccountGroupList[$accountId];

        $accountInfo->point         = isset($sonAccountTotal->point) ? $sonAccountTotal->point : 0;
        $accountInfo->used_point    = isset($sonAccountTotal->used_point) ? $sonAccountTotal->used_point : 0;
        $accountInfo->frozen_point  = isset($sonAccountTotal->frozen_point) ? $sonAccountTotal->frozen_point : 0;
        $accountInfo->overdue_point = isset($sonAccountTotal->overdue_point) ? $sonAccountTotal->overdue_point : 0;
     ...
     */
    public function QueryBatch($accountList, $overdueTime = null)
    {
        ...
        if ($accountList->count() > 0) {
            ...
            foreach ($accountList as $key => $accountInfo) {
                ...
                $mainAccounts[] = $accountInfo->account;
            }

            $curTime = time();

            $overdueTime = $overdueTime ?? $curTime;

            $earliestList        = SonAccountModel::GetEarliestOverdueTime($accountIds);
            $accounts = SonAccountModel::QueryByAccountBns($mainAccounts);
            $allSons = array();
            foreach ($accounts as $tmp_key=>$sonDetail){
                $allSons[$sonDetail->account_id][$sonDetail->son_account_id] = $sonDetail;
            }
            $sonAccountGroupList = SonAccountModel::GetAccountTotalValidPoint($accountIds, $overdueTime);
            foreach ($accountList as $key => $account) {
                ...
                $accountList[$key]->sons = isset($allSons[$accountId]) ? array_values($allSons[$accountId]) : [];
            }
        }
