...
class RuleRegionGps
{
    ...
    {
        try {
            $id = app('api_db')->table('server_delivery_rule_region_gps')->insertGetId($regionInfo);
        } catch (\Exception $e) {
            $id = false;
        }
        return $id;
    ...
    public static function DeleteByRegionId($regionId)
    {
        try {
            $status = app('api_db')
                ->table('server_delivery_rule_region_gps')
    ...
    {
        $model = app('api_db')->table('server_delivery_rule_region_gps')
            ->where('region_id', $regionId)
