...
//            print_r($this->_db_store->getQueryLog());die;
            ...
            while (1) {
                ...
                echo 'task:' . $task->task_id . ' rule_id:' . $task->rule_id . ' totel:' . $task->total . ' process:' . $task->process . '(' . implode(',',
                        $goods_ids) . ')' . "\n";
                // 处理完成
                if ($goods_ids === null) {
        ...
        switch ($condition['processor']) {
            case 'CostGoodsRuleProcessor': // 指定商品
                ...
                if (count($goods_ids) > 0) {
                    ...
                    $sql_count = 'select count(*) from sdb_b2c_goods WHERE goods_id in(' . implode(',',
                            $goods_ids) . ')';
                } else {
                    return null;
                }
            ...
            case 'CostBrandRuleProcessor': // 指定品牌
                ...
                if (count($brand_ids) > 0) {
                    $sql = 'select goods_id from sdb_b2c_goods WHERE brand_id in(' . implode(',',
                            $brand_ids) . ') AND goods_id>' . $max_goods_id . '
                     order by goods_id asc limit 0,' . $page_size;
                    $sql_count = 'select count(*) AS count from sdb_b2c_goods WHERE brand_id in(' . implode(',',
                            $brand_ids) . ')';
                }
                break;
  ...
  WHERE mall_goods_cat IN(' . $mall_cat_ids_str . ') AND goods_id>' . $max_goods_id . '
  ...
  WHERE m2g.mall_id IN(' . $mall_ids_str . ') AND goods.mall_goods_cat IN(' . $mall_cat_ids_str . ') AND m2g.goods_id>' . $max_goods_id . '
  ...
  LIMIT 0,' . $page_size;
                    ...
                    if (in_array('all', $condition['filter_rule']['mall_id_list'])) {
                        if (in_array('all', $condition['filter_rule']['mall_cat_id_list'])) { // “全部”分类
                            ...
                            $mall_cat_ids_str = $this->getLastLevelCatIdsStr($condition['filter_rule']['mall_cat_id_list']);
                            $sql = 'SELECT goods_id FROM sdb_b2c_goods goods
  WHERE mall_goods_cat NOT IN(' . $mall_cat_ids_str . ') AND goods_id>' . $max_goods_id . '
  ORDER BY goods_id ASC 
  LIMIT 0,' . $page_size;
                            $sql_count = 'SELECT count(*) AS count FROM sdb_b2c_goods 
  WHERE mall_goods_cat NOT IN(' . $mall_cat_ids_str . ')';
                        ...
                        if (!in_array('all', $condition['filter_rule']['mall_cat_id_list'])) { // 非全部分类
                            ...
                            $sql = 'SELECT m2g.goods_id FROM mall_module_mall_goods m2g
  LEFT JOIN sdb_b2c_goods goods ON goods.goods_id = m2g.goods_id
  WHERE (m2g.mall_id NOT IN(' . $mall_ids_str . ') OR goods.mall_goods_cat NOT IN(' . $mall_cat_ids_str . ')) AND m2g.goods_id>' . $max_goods_id . '
  ORDER BY m2g.goods_id ASC
  LIMIT 0,' . $page_size;
                            $sql_count = 'SELECT count(*) AS count FROM mall_module_mall_goods m2g
  LEFT JOIN sdb_b2c_goods goods ON goods.goods_id = m2g.goods_id
  ...
  ORDER BY goods_id ASC
  LIMIT 0,' . $page_size;
                            $sql_count = 'SELECT count(*) AS count FROM mall_module_mall_goods
 ...
 ORDER BY products.goods_id ASC 
 LIMIT 0,' . $page_size;
                $sql_count = null;
                break;
